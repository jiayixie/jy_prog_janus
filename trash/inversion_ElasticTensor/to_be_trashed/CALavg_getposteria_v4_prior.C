...
// this version, modified on Mar17, 2014. modified from v4, and it won't use misfit to reselect data. all data will be post. I just use this to put the model at certain depth, so it would be consisitent with the post file output from inversin.
//
#include<iostream>
#include<algorithm>
#include<vector>
...
//----------------------------------------------------- 
	...
	//get the average parameters from a list of acceptable paras
	    int i,j,k,size,Ngood,ibadp,idmin=0;
	    double mismin=1e10;
 	    vector<int> badv;
...
// this version, we will calculate the uncertainty of anisotropy------	
  ...
  while(1){
    ...
    idmin=para_avg_multiple_gp(idphi,paraall,parabest,paraavglst,parastdlst,idlstlst);
...
/*
    ...
    fclose(out);
	*/
    //---write out the best fitting model v(h)---- (not para)
    if((out=fopen(fmodBnm,"w"))==NULL){printf("### Cannot open %s to write!!\n",fmodBnm);exit(0);};
    float h=0.;
