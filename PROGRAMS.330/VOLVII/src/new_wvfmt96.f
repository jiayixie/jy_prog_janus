...
c-----
     ...
     1      nzyear,nzjday, nzhour, nzmin, nzsec, nzmsec, mshft_store
        integer wvtype(MXWVS), mshft(MXWVS), npts(MXWVS), douse(MXWVS)
        integer itotal_pts, mshft_store(MXWVS)
...
c-----
        ...
        call getevent_data(obs_name(1),ounit,'new_wvfmtd96')
c-----
...
c-----
     ...
     7          nzsec(i), nzmsec(i))      
        call shiftall(mshft(i),npts(i),bg(i),bo(i),dt(i),i,ipass)
...
c
CDEBUG          write(ounit,*) ' Sum of Square Amplitudes: ',sum_sq_amp(i)
CDEBUG          write(ounit,*) ' Weight:  ',obswt(i)
CDEBUG          write(ounit,*) ' '
c-----
c
...
c-----
c        do 141 j=1,npts(i)
c            a(j + ibegin -1,1) = a(j + ibegin -1,1) - a(j + ibegin -1,6)
c            a(j + ibegin -1,2) = a(j + ibegin -1,2) - a(j + ibegin -1,6)
c            a(j + ibegin -1,6) = 0.0
c  141   continue
...
c-----
C        mtensor(6) = -(mtensor(1) + mtensor(2))
...
c-----
    ...
    2   format(a8,1x,a8,1x,f6.3,e10.3,f6.3,e10.3,e10.3,f10.2,f10.2)
            write(ounit,2)kstnm(i), kcmpnm(i),obswt(i),
     1          err,err/sum_sq_amp(i),factor,misfit(i)
     2          ,mshft(i)*dt(i),kshft_store(i)*dt(i)
c 
            ibegin = ibegin + npts(i)
c
...
c-----
    ...
    3   format('new_WVFMT96 ',f6.1,3f6.0,f7.2,f10.3,e10.3,
     1     2f10.3,e10.3,f6.1)
        call outmsg(nwaves)
        close (3)
        endif
        ...
        subroutine docross(o,p,npts,sumerr,xmis,mshft,iwave,ipass)
        implicit none
        integer MAXPTS, NPRM, NMAX, MXWVS
        parameter(MAXPTS=2048, NPRM=6, MXWVS=1000, NMAX=MXWVS*maxpts)
c-----
...
CRBH        if(ipass.eq.1)then
CRBHc-----
CRBHc             calculate the envelope
CRBHc-----
CRBH              call do_envelope(o,oe,npts)
CRBH              call do_envelope(p,pe,npts)

CRBH        else
              call h_copy(o,oe,npts)
              call h_copy(p,pe,npts)
CRBH        endif
c-----
...
c-----
        ...
        subroutine shiftall(mshft,npts,bg,bo,dt,iwave,ipass)
        integer mshft, npts, iwave, ipass
c-----
c       apply the shift to the data stream 
...
c                       and bg
c       npts - number of time samples in series 
...
c-----
     ...
     1                response(MAXPTS)
        ...
        common mshft_store
        real obs
        integer i, j, n, mshft_store(MXWVS)
        if(ipass.eq.3) mshft=mshft+mshft_store(iwave)
        if(mshft.eq.0)then
            return
...
c-----
        ...
        subroutine preobssv(i,ibegin, ipass)
        implicit none
        integer i
...
c-----
            ...
            if(ipass.eq.3)then
            wvfile(1:slen+7) = obs_name(i)(1:slen)//'.pre.03'
            else
            wvfile(1:slen+7) = obs_name(i)(1:slen)//'.pre.02'
            endif
            if(douse(i).eq.1)then
c-----
...
c-----
            ...
            slen = lgstr(obs_name(i))
            if(ipass.eq.3)then
            wvfile(1:slen+7) = obs_name(i)(1:slen)//'.obs.03'
            else
            wvfile(1:slen+7) = obs_name(i)(1:slen)//'.obs.02'
            endif
            if(douse(i).eq.1)then
