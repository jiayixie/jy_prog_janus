...
c       taken to be 0.95 of the surface ray parameter
c-----
        pmin = 0.0
        if(kpar.eq.1)then
            pmax = 0.95/a(1)
...
c-----
                call ptau(0.0,kpar,m,iret,time,dist,hs,rcoef)
                call gwidth(wid*abs(rcoef))
                do 1020 i=1,NRAY
                    p = pmin + ( i-1)*dp
                    call ptau(p,kpar,m,iret,time,dist,hs,rcoef)

                    if(iret.gt.0)then
                        time = time +p*dist 
C        if(i.eq.10)then
C            write(6,*)'Layer,multiple,(r,t):',m,j,dist,j*time
C        endif
                        if(vred.gt.0.0)then
                            time = time - dist/vred
                        endif
                        xx = x0 + j*(dist-xmin)*dx
                        yy = y0 + j*(time-tmin)*dy
 ...
 1000   continue
        ...
        subroutine ptau(p,kpar,mm,iret,time,dist,hs,rcoef)
c-----
c       p   R   - ray parameter
c       kpar    I   - 1 P, 2 SV, 3 SH
...
c-----
        ...
        do 1000 i=1,mm
            ...
            if(hs .gt. zboundary)then
                 dist = dist + 1*d(i)*pv/sr
                 time = time + 1*d(i)*sr/v
            else
                 dist = dist + 2*d(i)*pv/sr
                 time = time + 2*d(i)*sr/v
            endif
CC                pv = p*v
CC                dist = dist + 2*d(i)*pv/sqrt(1.0-pv*pv)
CC                time = time + 2*d(i)*sqrt(1.0-pv*pv)/v
