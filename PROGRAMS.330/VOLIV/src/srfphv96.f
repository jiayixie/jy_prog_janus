...
c       08 DEC 2012 When plotting Love and Rayleigh dispersion, use the
c                   same scale
c       13 APR 2013 Actually implemented the fix of 08 DEC 2012
c-----
c
...
c-----
     ...
     1      ,ilvry,ibcx,ibcy,ncx,ncy,mmax,.false.)
        else
c-----
c           plot Love
c-----
            ...
            yaxlen = 4.0
        call vmat(id,wref,invcsl,nf1,x0,y0,xaxlen,yaxlen
     1      ,1,ibcx,ibcy,ncx,ncy,mmax,.true.)
c-----
c           plot Rayleigh
            x0 = 6.5
            y0 = 2.0
            ...
            yaxlen = 4.0
        call vmat(id,wref,invcsl,nf1,x0,y0,xaxlen,yaxlen
     1      ,2,ibcx,ibcy,ncx,ncy,mmax,.true.)
        endif
        call pend()
        close(1,status='keep')
        end
     ...
     1      ,ilvry,ibcx,ibcy,ncx,ncy,m,dopair)
c-----
c     a general purpose reformatting file
c       id        I - code that indicates inversion type
c       ilvry     I - 1 for Love, 2 for Rayleigh
c       x0        R - 
c       y0        R - (x0,y0) define lower left corner of plot
c       xaxlen    R - length of X axis
c       yaxlen    R - length of Y axis
c       ibcdx     C - X-axis label
c       ibcdy     C - Y-axis label
c       ncx       I - number of characters is X-Axis label
c       ncy       I - number of characters is Y-Axis label
c       m         I - number of layers in model
c       dopair    L - .true. use both L and R to get the scale
c-----
        logical dopair
        parameter(LER=0,LIN=5,LOT=6)
...
c-----
c           jrun = 1  - get bounds
c                       if dopair = .true. consider both L and R
...
c-----
	    ...
	    if(jrun.eq.1)then
	    if(ifn.ne.ilvry .and. .not. dopair )go to 10
	    else 
