...
int sac_add ( SAC_HD *shd, float *fsig, char *filename, int flag, float wei ) {
  ...
  if(shd->user8<-12000.){NN=0;}
  else{NN=shd->user8;}
  if(shd1.user8<-12000.){NN++;}
  else{NN=NN+shd1.user8;}
...
//    fprintf (stderr,"reverse!!!!!\n");
    ...
    fsig[3000] = fsig[3000] + tsig[(ns-1)/2]*wei;    
    shd->user9=shd1.cmpaz;
    shd->cmpaz=shd1.user9;
    }

  //printf("cmpaz=%g %g; user9=%g %g; %s\n",shd1.cmpaz,shd->cmpaz,shd1.user9,shd->user9,filename);
  shd->user4=Ncount;
  shd->user8=NN;
  return 1;
  }
...
int main (int argn, char *argv[]) {
  ...
  char tname3[300],tname4[300],ch[4][10],chrv[4][10];
  ...
  nsta = i;
  fclose(ff1);  
  sprintf(ch[0],"NN");sprintf(chrv[0],"NN");
  sprintf(ch[1],"EE");sprintf(chrv[1],"EE");
  sprintf(ch[2],"NE");sprintf(chrv[2],"EN");
  sprintf(ch[3],"EN");sprintf(chrv[3],"NE");
...
//       jjjj = 0;
	 ...
	 if (k>=3){ /// -------------- modify parameter here ----------------------
	  sprintf(tname1,"%s/%s/COR_%s_%s.SAC_%s\0",dirlist[k],staname2[i],staname2[i],staname2[j],ch[nch]);
         sprintf(tname2,"%s/%s/COR_%s_%s.SAC_%s\0",dirlist[k],staname2[j],staname2[j],staname2[i],chrv[nch]);
//	  printf(" #### test k>=1 name=%s  %s\n",tname1,tname2);
//	  exit(0);
	 }// if k>1
 	 else{
         sprintf(tname1,"%s/%s/COR_%s_%s.SAC_%s\0",dirlist[k],staname[i],staname[i],staname[j],ch[nch]);
         sprintf(tname2,"%s/%s/COR_%s_%s.SAC_%s\0",dirlist[k],staname[j],staname[j],staname[i],chrv[nch]);
	 } //else
