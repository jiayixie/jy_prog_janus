...
//--------------------------
int para_avg_multiple_gp(int idphi,int idphiM, vector<paradef> &paralst, paradef &parabest, vector<paradef> &paraavglst, vector<paradef> &parastdlst, vector<vector<int> > &idlstlst, int flag){
  ...
  vector<int> indexflaglst,indexflaglstM,idlst,idlstnew;
  ...
  */
  ...
  for(i=0;i<Ngp;i++){
	for(j=0;j<NgpM;j++){
  		...
  		for(k=0;k<Ngood;k++){
			if(indexflaglst[k]==i+1 and indexflaglstM[k]==j+1)idlstnew.push_back(idlst[k]);
		}//for k<Ngood
		printf("====the %dth group has %d models=====\n",i*Ngp+j,idlstnew.size());//--check--
