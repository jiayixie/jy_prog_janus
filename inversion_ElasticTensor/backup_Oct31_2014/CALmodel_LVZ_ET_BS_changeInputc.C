...
// this version, change the line 481, change the value of input eta, so that the input c would be changed, modified Mar 7, 2014
using namespace std;
/*=====CONTENT=======
...
//----------------------------------------------------- 
	int readmodAniso(modeldef &model,const char *name){
	//modified Mar 7, 2014, for pflag==1, changed the eta value based on the ellipticity parameter e
	  float vsv,vsh,vpv,vph,eta,rho,A,C,L,e;
	  string line;
		  ...
		  for(i=0;i<tnp;i++)
		 	{
			  ...
			  if(pflag>0){ 	  	
				...
				model.groups[iid].thetavalue.push_back(0.);
			      ...
			      if(0){
  			      //if(iid==1){
			      e=2; //the ellipticity para
			      vsv=model.groups[iid].vsvvalue[i];
			      vsh=vsv;
			      vpv=vsv*model.groups[iid].vpvs;
		   	      vph=vpv;
			      rho=0.541+0.3601*0.5*(vpv+vph);
			      A=rho*vph*vph;
			      C=rho*vpv*vpv;
			      L=rho*vsv*vsv;
			      eta=(A+C-4*L-8*e)/2/(A-2*L);
			      //printf("group%d, eta=%g\n",iid,eta);
			      model.groups[iid].etavalue.push_back(eta);}
			      //*/
			      //if(iid==1){
			      //model.groups[iid].etavalue.push_back(0.9);}
			      else
			      {model.groups[iid].etavalue.push_back(1.0);}
			      //-------------
			      model.groups[iid].thetavalue.push_back(0.);
