...
int main(int na, char *arg[])
{
  if(na!=8)
    {
      cout<<"usage:travel_time_to_velocity_map event.lst azi_min azi_max N_bin out_name dx"<<endl;
      return 0;
    }
  FILE *ff,*fin,*fout,*file1,*file_iso,*file_ani;
  int i,j,k;
  int npts_x,npts_y;
  char buff1[300],sta1[10],name_iso[100],name_ani[100],name_ani_n[100],dirph[200];
...
//      if(fscanf(file1,"%s %lf %lf %lf %lf",&event_name,&dump1,&dump2,&dump3,&dump4)==EOF)
  ...
  for(;;)
    {
	//cout<<"begin to read file "<<buff1<<endl;
...
//fprintf(stderr,"%f,%f,%f,%f,%d,%d\n",x0,x1,y0,y1,npts_x,npts_y);
//return 1;

  fprintf(stderr,"Memory check!! npts_x=%d npts_y=%d\n",npts_x,npts_y);
  double slow[npts_x][npts_y][2000],slow_weight[npts_x][npts_y][2000];
...
//      if(fscanf(file1,"%s %lf %lf %lf %lf",&event_name,&dump1,&dump2,&dump3,&dump4)==EOF)
  ...
  cout<<"write "<<name_iso<<endl;//test---
  nsta=20; //20  // PARAMETER : NSTA This nsta is different from that above
  double w2;
...
//  among all events that covers point i j, for each event k, count how may events are there within 25 degree,N[k] => weight[k]=1/N[k]*slow_weightijk=1/N[k];
  ...
  nstep1 = 4; // the region for average ani would be a rectangle with length (grid*nstep1*2)
  nstep2 = 2; // the points used to average ani would be (nstep1*2/nstep1+1)**2
  npoint = 15;
  for(i=0;i<npts_x;i++)
    {
      for(j=0;j<npts_y;j++)
