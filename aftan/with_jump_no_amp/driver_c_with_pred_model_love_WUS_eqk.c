...
int main (int argc, char *argv[])
{
  ...
  char sta1[20], sta2[20],nt1[5],nt2[5];
...
// input command line arguments treatment
  if(argc != 3) {
      printf("Usage: aftan4_c_test parameter_file wrong_path_file\n");
      exit(-1);
  }
 ...
 */
	//./20110804135134/SAC_rot/AZ.MONP2..t
	//20050710044629.TA.O03C.LHT.sac
    sscanf(root,"%[0-9].%[A-Z,a-z,0-9].%[A-Z,a-z,0-9].",sta1,nt2,sta2);
   // printf("evt=%s  nt.st=%s.%s root=%s\n",sta1,nt2,sta2,root);
//   sscanf(root, "COR_%[A-Z,a-z,0-9].%[A-Z,a-z,0-9]_%[A-Z,a-z,0-9].%[A-Z,a-z,0-9].", nt1,sta1,nt2,sta2);
//	  sscanf(root,"%[0-9].%[A-Z,a-z].",sta1,sta2);
	  printf("======evnm:%s  stanm:%s.%s root:%s\n",sta1,nt2,sta2,root);
	//sprintf(phvelname,"/projects/jixi7887/work/US/PREDICTION_eqk_cvlst/L/%s/%s.%s.%s.dat",sta1,sta1,nt2,sta2);
	sprintf(phvelname,"/lustre/janus_scratch/jixi7887/US/pred_Lov/PREDICTION_eqk_cvlst/L/%s/%s.%s.%s.dat",sta1,sta1,nt2,sta2);
  int jy = 0;
  if((inv=fopen(phvelname,"r"))==NULL){
	/*printf("%s\n",phvelname);
	sprintf(phvelname,"/media/CHINA/jiayi/WUS/WUS_predict_love/PREDICTION_eqk/evt_sta_pair/L/%s.%s.%s.dat",sta1,nt2,sta2);
 	if ( (inv=fopen(phvelname,"r"))==NULL ){
	printf("%s\n",phvelname);
	sprintf(phvelname,"/media/CHINA/jiayi/WUS/WUS_predict_love/PREDICTION_eqk_part2_04_07/evt_sta_pair/L/%s.%s.%s.dat",sta1,nt2,sta2);
	if ((inv=fopen(phvelname,"r"))==NULL) {
	printf("%s\n",phvelname);
   	jy=1;nprpv=0;printf("ref_disp doesn't exist, %s\n",phvelname);
	fprintf(fwrong,"%s %s.%s %s\n",sta1,nt2,sta2,root);
	continue;
	}}
	*/
	fprintf(fwrong,"%s %s.%s %s\n",sta1,nt2,sta2,root);
  ...
  //continue;
   /* //sprintf(phvelname,"/media/CHINA/jiayi/WesternChina/model/PREDICTION/evt_sta_pair/L/%s.%s.%s.%s.dat",nt2,sta2,nt1,sta1);
	sprintf(phvelname,"/media/CHINA/jiayi/WUS/WUS_love/PREDICTION_2013/evt_sta_pair/L/%s.%s.%s.%s.dat",nt2,sta2,nt1,sta1);
    if((inv = fopen(phvelname,"r")) == NULL){
	printf("Can not find file %s\n",phvelname);
