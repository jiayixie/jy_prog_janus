...
int write_ASC(modeldef model,paradef para, char *namemod, char *Rnamedisp,char *Lnamedisp,char *AZRnamedisp, char *AZLnamedisp, int Rsurflag, int Lsurflag, int RAZampflag, int LAZampflag,int RAZphiflag, int LAZphiflag)
{
 ...
 fprintf(outmod,"L %8g  RL %8g %8g %8g LL %8g %8g %8g  \nmisfit %8g Rm %8g %8g %8g Lm %8g %8g %8g\n",model.data.L,model.data.Rdisp.L,model.data.AziampRdisp.L,model.data.AziphiRdisp.L,model.data.Ldisp.L, model.data.AziampLdisp.L,model.data.AziphiLdisp.L,  model.data.misfit, model.data.Rdisp.misfit, model.data.AziampRdisp.misfit, model.data.AziphiRdisp.misfit,model.data.Ldisp.misfit, model.data.AziampLdisp.misfit,model.data.AziphiLdisp.misfit);
  ...
  fprintf(outmod,"\n");
  fprintf(outmod,"%10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s\n","depth","vsv","vsh","vpv","vph","theta","phi","qs","qp","rho","vpvs");
  for(i=0;i<model.laym0.nlayer;i++){
  	fprintf(outmod,"%10g %10g %10g %10g %10g %10g %10g %10g %10g %10g %10g\n",h,model.laym0.vsv[i],model.laym0.vsh[i],model.laym0.vpv[i],model.laym0.vph[i],model.laym0.theta[i],model.laym0.phi[i],model.laym0.qs[i],model.laym0.qp[i],model.laym0.rho[i],model.laym0.vpvs[i]);
