...
# Oh, I just realized that for earthquake, the way i measure snr has problem (signal/tail_noise). for some eqk that's far, tail may contain some signal, so the snr would be under-estimated. So,  maybe use a constant end_period is more reasonable for eqk (since eqk are longer periods, it's not easy to get 2pi ambiguity, I think)
...
##################### parameter ###########################
snr_cri = 5.0
eTmax = 100.0 # the maxT of model is 50sec  the minT of model is 8sec
eTmin = 50.0
bT = 14.0
