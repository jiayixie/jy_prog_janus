...
/*--------------------------------------------------------------*/
{
  FILE *ff,*ff1;
  char filename[500];
  char stanm[2000][10],dir[100],str[500];
  int i;
  float dump;
//check input command line arguments
  if(argc != 4) {
    fprintf(stderr,"USAGE: spectral_snr_f [stack_rot_dir] [stanm.lst] [component s, n, or p]\n");
  ...
  for(i=0;;i++)
    {
      if(fscanf(ff,"%s %g %g",stanm[i],&dump,&dump)==EOF)
	break;
      fprintf(stderr,"----working on sta %d %s component=%s\n",i,stanm[i],argv[3]);
      sprintf(str,"rm -f temp.lst\nls %s/%s/*_%s > temp.lst\n",dir,stanm[i],argv[3]);
      system(str);
      if ((ff1=fopen("temp.lst","r"))==NULL)
      {
	fprintf(stderr,"Problem! cannot open cor_sac list\n");
	exit(1);
      }
      for (;;){
      
      if(fscanf(ff1,"%s",filename)==EOF)
	break;
