...
{ // based on given period (the input period) and Mineos_T and Mineos_vel, interpolat Mineos_vel into values at the given periods
  ...
  inper=indisp.hvper;
  outdisp.hvratio.clear();
  refperiod.clear();
  if(outdisp.fhv>0){
    refperiod=outdisp.hvper;
    n=outdisp.nhvper;
    k=indisp.nhvper;
    if (inper[0]<refperiod[n-1] or inper[k-1]>refperiod[0])
    {
    ...
    for(i=0;i<n;i++){
      ...
      for(j=k-1;j>-1;j--){
        if(inper[j]<Tref)continue;
        thvratio=(indisp.hvratio[j]-indisp.hvratio[j-1])/(inper[j]-inper[j-1])*(Tref-inper[j-1])+indisp.hvratio[j-1];
        outdisp.hvratio.push_back(thvratio);
        k=j-1;//k is changing!
        m++;
	break;
      }//for j
...
*/
//--------------------------------------
int read_dispMineos(dispdef &indisp,const char* Moutputnm, const char* HVoutputnm, int Nmod)
{//from Minoes output(Moutputnm with Nmod lines of model), read in the dispersion information to disp(indisp)
  ...
  // reads in the phase/group velocity dispersion curve information
  Mf.open(Moutputnm);
  ...
  if(indisp.fhv>0){
  // reads in the HV ratio dispersion curve information
  Mf.open(HVoutputnm);
  if(not Mf.is_open()){cout<<"### in read_dispMineos, cannot open HVoutput file"<<HVoutputnm<<endl;exit(0);}
  i=0;
  tdisp.hvratio.clear();tdisp.hvper.clear();
  tdisp.nhvper=0;
  while(getline(Mf,line)){
    i++;
    v.clear();
...
//--------------------------------------
int compute_diff(dispdef disp1, dispdef disp2, vector<vector<double> > &pveldiff,vector<vector<double> > &gveldiff, vector<vector<double>> &hvratiodiff)
{//veldiff: [T,v1,v2,v1-v2]
  ...
  //get diff
  pveldiff.clear();gveldiff.clear();hvratiodiff.clear();
  ...
  for(i=0;i<disp1.nhvper;i++){
	//***********check*********
	if(pow(disp1.hvper[i]-disp2.hvper[i],2)>0.1){printf("## in compute_diff for HV ratio, period lists don't match!\n###T1=%g T2=%g\n",disp1.hvper[i],disp2.hvper[i]);exit(0);}
	//************************
	tdiff.clear();
	tdiff.push_back(disp1.hvper[i]);
	tdiff.push_back(disp1.hvratio[i]);
	tdiff.push_back(disp2.hvratio[i]);
	tdiff.push_back(disp1.hvratio[i]-disp2.hvratio[i]);
	hvratiodiff.push_back(tdiff);
  }// for i nhvper
...
//---------------------------------------
int compute_Vkernel_single_para(paradef para, int i,modeldef model, vector<vector<double> > PREM,int Nprem, int Rflag, int Lflag,int flagupdaterho, vector<double> &trkp1, vector<double> &trkg1,vector<double> &trkhv1, vector<double> &tlkp1, vector<double> &tlkg1, int ipara){
 ...
 if(Rflag>0 and Lflag==0){
	...
	for(j=0;j<model.data.Rdisp.nhvper;j++)
		trkhv1.push_back(DRhvratio[j][3]/ddp);
...
//num_threads(2)
  ...
  for(int i=0;i<para.npara;i++){
	...
	*/
	...
	if(LVflag==1 and para.space1[i][2]*(para.space1[i][2]-0.001)<0.){// this para usese Vpara&Vkernel to do fwd computation and para don't need to be perturbed or scaled; then no need to cpt its partial deriv
		...
		if(ppflag==10 or ppflag==11){// by default, ppflag is increasing, so ppflag==1 for this group has kernel computed already , BUT ATTENTION, THIS PART NEED CHANGE IN PARALLEL CASE!
			...
			*/
			...
			compute_Vkernel_single_para(para,i,model,PREM,Nprem, Rflag,Lflag,flagupdaterho,trkp1,trkg1,trkhv1,tlkp1,tlkg1,i+ijump*para.npara);
       ...
       //else if( model.groups[ng].flagcpttype==1 or model.groups[ng].flag==1 or model.groups[ng].flag==6 ){// cpt with Vkernel, or layered model, or point model; BS
       	else if (model.groups[ng].flagcpttype==4 and model.groups[ng].flag!=2){// all cpt with Lkernel, and group is not Bspline 
			trkp1=Rp0;trkg1=Rg0;tlkp1=Lp0;tlkg1=Lg0;trkhv1=Rhv0;
                printf("@@@ check, compute_Vkernel, dealing with the %d/%dth para, NO Vkernel cpt=====\n",i,para.npara-1);		
       	}//if 4
	...
	else{// Modify: I think I need to add the flagcpttype==2 situation (Vk--> RA; Lk--> AZ)
  ...
  for(int i=0;i<para.npara;i++){// this part is especially for parallel running case; get Vkernel for AZcos, AZsin after all Vkernel have been computed.
	...
	if(model.groups[ng].flagcpttype==3){
		if(ppflag==1){//Vsv
			...
			temp3=tlkp2[i];
			temp4=tlkg2[i];
			temp5=trkhv2[i];//this is probably useless for AZdisp computation
		}
...
//      kRp=new double [nP][nT];
  ...
  if (model.data.Rdisp.fhv>0){
        ...
        for(i=0;i<nP;i++){
          tkp1.clear();
          for(j=0;j<nT;j++)
                tkp1.push_back(kRhv[i][j]);
          tkp2.push_back(tkp1);
        }//for i nP     
        kernel.push_back(tkp2);
        for(i=0;i<nP;++i)delete [] kRhv[i];
        delete [] kRhv;
  }//if Rflag>0 and fhv>0
  else{
        kernel.push_back(ttk2);}
...
// only do this to point(from BS model) or layered model (model.group[].flag=1 or 6)
  ...
  for (int i=0;i<para.npara;i++){
	...
	if(LVflag==1 or model.groups[ng].flagcpttype==3 or model.groups[ng].flagcpttype==1){
		printf("@@@ check, compute_Lkernel, dealing with the %d/%dth para, NO Lkernel cpt=====\n",i,para.npara);
 		trkp1=Rp0;trkg1=Rg0;trkhv1=Rhv0;tlkp1=Lp0;tlkg1=Lg0;}// only need Vkernel,so Lkernel-->0; or cpt with V+AZ kernel; or cpt with Vkernel
