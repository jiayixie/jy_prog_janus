...
c               unphysical
c       24 JAN 2007 - added -TIMEP pfile to plot P first arrival times 
c       24 JAN 2007 - added -TIMES sfile to plot S first arrival times
...
c-----
     ...
     2      doleg,kvreds,pfile,sfile)
        if(nmod.gt.0)then
c-----
c       open output text file
c-----
        open(2,file='TIMMOD96.TXT',access='sequential',
     1      form='formatted',status='unknown')
        rewind 2
c-----
c       open plot file
c-----
            call pinitf('TIMMOD96.PLT')
            call newpen(1)
...
c-----
            ...
            do 100 i=1,NDIST
                dist = xmin + ( i-1)*dx
                r(i) = dist
            call frstar(dist,hs,hr,mname(m),1,tp (i),pvel,svel,den,
     1           vsa, vsb, vsr, rayp, geom, tstar, .false.,.false.)
            call frstar(dist,hs,hr,mname(m),2,tsv(i),pvel,svel,den,
     1           vsa, vsb, vsr, rayp, geom, tstar, .false.,.false.)
            call frstar(dist,hs,hr,mname(m),3,tsh(i),pvel,svel,den,
     1           vsa, vsb, vsr, rayp, geom, tstar, .false.,.false.)
            write(2,'(4f10.3)')dist,tp(i), tsv(i), tsh(i)
            if(vred.gt.0)then
                tp(i)  = tp(i)  - dist/vred
 ...
 100        continue
     ...
     1          r,tp ,NDIST,wid,pfile,vred,kpar)
            else if(kpar.eq.2)then
            call doplttim(x0,y0,xlen,ylen,xmin,xmax,tmin,tmax,
     1          r,tsv,NDIST,wid,sfile,vred,kpar)
            else if(kpar.eq.3)then
            call doplttim(x0,y0,xlen,ylen,xmin,xmax,tmin,tmax,
     1          r,tsh,NDIST,wid,sfile,vred,kpar)
            endif
            if(doleg)then
                call newpen(ipen)
...
c-----
     ...
     1      r,tp,NDIST,wid,phasefile,vred,kpar)
        implicit none
        ...
        integer kpar
c-----
c       phasefile   file with phase travel time each line is a distance - time pair
c       vred        reduction velocity if > 0
c       kpar        1- P, 2 - SV, 3 -SH
c-----
...
c-----
 ...
 100    continue
        call gwidth(0.0)
c-----
        if(phasefile .ne. ' ')then
              open(3,file=phasefile, access='sequential',
     1            form='formatted', status='unknown')
              rewind 3
 1000         continue
                   read(3,*,end=9000)dist, time
                   if(vred.gt.0.0)then
                         time = time - dist/vred
                   endif
                   if(dist.ge.xmin .and. dist.le.xmax .and.
     1                  time.ge.tmin .and. time.le.tmax)then
                        xx = x0 + ( dist - xmin)*dx
                        yy = y0 + ( time - tmin)*dy
                        if(kpar.eq.1)then
                             call curvit('CI',0.05,xx,yy)
                        else if(kpar.eq.2)then
                             call curvit('DI',0.05,xx,yy)
                        else if(kpar.eq.3)then
                             call curvit('SQ',0.05,xx,yy)
                        endif
                   endif
              go to 1000
 9000         continue
...
c       pfile   Ch*100 - file of distance P first arrival travel time
c       sfile   Ch*100 - file of distance S first arrival travel time
c-----
        implicit none
 ...
 1000   continue
            ...
            if(name(1:2).eq.'-K')then
                ...
                doleg = .true.
            else if(name(1:6).eq.'-TIMEP')then
                i = i + 1
                call mgtarg(i,pfile)
            else if(name(1:6).eq.'-TIMES')then
                i = i + 1
                call mgtarg(i,sfile)
 ...
 2000   continue
     ...
     3      '-K kolor -VRED vred -HS hs -HR hr -LEG ',
     4      '-TIMEP pfile -TIMES sfile model96_file[s]'
     ...
     1  '-LEG       (default none) Put in file legend'
        write(LER,*)
     1  '-TIMEP pfile (default none) plot (dist,time) pairs'
        write(LER,*)
     1  '-TIMES sfile (default none) plot (dist,time) pairs'
        write(LER,*)
