...
for i in range (len(names)):
	...
	for ll in open(names[i],"r"):
		...
		if (ttime > 20.):
			break;
		time.append(ttime-0.);
		tbaz=int(float(ll1[1]))
		if(math.isnan(float(ll1[2]))):
			nanlst[i]=1
			flagnan=1
			break
		data.append(float(ll1[2]));
		c.append(0.);
		c1.append(0.);
		c2.append(0.);
		c3.append(0.);
...
print "number of non-nan data (# of baz) from input=%d,dt=%f"%(len(baz),dt)
...
# out put #########################################################
#--for each singla, write out its data-A0(time). the length of each output data is the same as the shortest data.
for i in range(len(nameoutlst)):
	fout=open(nameoutlst[i],"w")
	for j in range(min(lens)):
		fout.write("%7f %7f %7f\n"%(atime[i][j],baz[i],adata[i][j]-A0[j]));
	fout.close()
###################################################################
