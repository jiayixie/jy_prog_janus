...
#include"CALforward_Mineos_readK_BS.C_bak"
...
sprintf(tmpstr,"if [ ! -d %s/binmod ]; then mkdir %s/binmod; fi",dirlay,dirlay);
  ...
  while(1){
    ...
    if(model0.flag==0){updatemodel(model0,flagupdaterho);printf("update!\n");}
    /*---check---
    for(i=0;i<model0.ngroup;i++){
	printf("group%d\n",i);
	double h=0.;
	for(j=0;j<model0.groups[i].nlay;j++){h=h+model0.groups[i].thick1[j];}
	printf("\tthick=%g nlay=%d h=%g\n",model0.groups[i].thick,model0.groups[i].nlay,h);
    }
    */
    ...
    /*--check ---
    paraP=para1;modelP=model0;
    para1=paraBS;model0=modelBS;

    para1.parameter[1]=para1.parameter[1]*1.05;
    para1.parameter[8]=para1.parameter[8]*1.05;
    para1.parameter[9]=para1.parameter[9]*1.02;

    Bsp2Point(model0,para1,modelP,paraP,flagupdaterho);
    para1=paraP;model0=modelP;

    para2mod(para1,model0,model1);
    updatemodel(model1,flagupdaterho);
    compute_dispMineos(model1,PREM,Nprem,Rsurflag,Lsurflag);
    printf("\n@@@ check, main, from changed model\n");
    for(j=0;j<model1.data.Rdisp.npper;j++)printf("  @@@ check, T=%g,vin=%g vold=%g\n",model1.data.Rdisp.pper[j],model1.data.Rdisp.pvelo[j],model1.data.Rdisp.pvel[j]);
    */
