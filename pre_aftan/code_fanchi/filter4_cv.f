c ==========================================================
c Function filter4_cv. Broadband filtering.
c ==========================================================
c Parameters for filter4_cv function:
c Input parameters:
...
c output is the envelope function!!!!!!!!!!!!!!!!!!!!!!!!
c ==========================================================

      subroutine filter4cv(f1,f2,f3,f4,npow,dt,n,seis_in,seis_out)
c      implicit none
      include 'fftw3.h'
      integer*4 npow,n
...
cc change half spectra into the same value
      nk = ns/2+1
      do k = nk+1,ns
        sf(k) = czero
c         sf(k) = sf(ns-k)/2.0d0
      enddo
...
c      sf(1) = sf(1)/2.0
c      sf(nk) = dcmplx(dreal(sf(nk)),0.0d0)
      sf(nk) = sf(1)
c===============================================================
c   make tapering
...
c        seis_out(k) = 2.0*real(dreal(s(k)))/ns
        seis_out(k) = sqrt(2*(real(dreal(s(k)))*real(dreal(s(k))) + 
     *    REAL(dimag(s(k)))*REAL(dimag(s(k)))) )/n
c        print*, 'cv !! '
      enddo
      return
      end
