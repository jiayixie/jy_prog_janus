...
c
c
c     This program checks the input control file 'tobs.d' and
...
c-----
        call mchdep()
c-----
c     test for the existence of tobs.d
c     if it does not exist interactively set it up
c-----
        inquire(file='tobs.d',exist=ext)
        if(.not.(ext))    call settobs(a,b,d,r,rat,qbinv)
c-----
        open(1,file='tobs.d',status='old',access='sequential')
        rewind 1
C          read(1,*) h,dcl,onel,dcr,oner
...
c
C        call getdsp(nmdisp,nf,dcl,dcr,onel,oner,h,iunitd)
C              nf34=max(nf(3),nf(4))
C              nf67=max(nf(6),nf(7))
c-----
...
c-----
       ...
       return
        ...
        subroutine settobs(a,b,d,r,rat,qbinv)
c-----
c     set up the control file tobs.d interactively
c-----
        ...
        enddo
c-----
c     open the file tobs.d
c-----
        open(1,file='tobs.d',status='unknown',form='formatted',
     1            access='sequential')
        rewind 1
c-----
...
C     1 ' Enter h,dcl,dcr'
C        write(LOT,*)
C     1 '      h = fraction change in period to get group vel'
C        write(LOT,*)
C     1 '            (0.005 is reasonable)'
C        write(LOT,*)
C     1 '      dcl, dcr are phase velocity increment in root'
C        write(LOT,*)
C     1 '            search for Love and Rayl respectively'
C        read(LIN,*)h,dcl,dcr
C        onel = 0.0
C        oner = 0.0
C        write(1,*)h,dcl,onel,dcr,oner
Cc-----
Cc     get second line of tobs.d
Cc-----
C        write(LOT,*)
...
C        read(LIN,*)nf(1)
C        write(LOT,*)
C     1 ' Enter maximum number of Love gamma modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS LOVE gamma data'
C        read(LIN,*)nf(2)
C        write(LOT,*)
C     1 ' Enter maximum number of Love Phvel modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS LOVE phase vel data'
C        read(LIN,*)nf(3)
C        write(LOT,*)
C     1 ' Enter maximum number of Love Gpvel modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS LOVE group vel data'
C        read(LIN,*)nf(4)
C        write(LOT,*)
C     1 ' Enter maximum number of Rayl gamma modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS RAYL gamma data'
C        read(LIN,*)nf(5)
C        write(LOT,*)
C     1 ' Enter maximum number of Rayl Phvel modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS RAYL phase vel data'
C        read(LIN,*)nf(6)
C        write(LOT,*)
C     1 ' Enter maximum number of Rayl Gpvel modes to process'
C        write(LOT,*)
C     1 '      0 means DO NO PROCESS RAYL group vel data'
C        read(LIN,*)nf(7)
C       write(LOT,*)
...
c-----
C        write(LOT,*)
C     1 ' Enter name of dispersion file'
C        read(LIN,'(a)')nmdisp
C        write(1,'(a)')nmdisp
c-----
c     if the file does not exist, then interactively build it
c-----
C        inquire(file=nmdisp,exist=ext)
C        if(.not.(ext))call setdsp(nmdisp)
c-----
