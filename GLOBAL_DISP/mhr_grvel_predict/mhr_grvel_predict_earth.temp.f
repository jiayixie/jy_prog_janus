	     parameter ( maxpath = 100000) 

	     character*180 map_name,pathfile_name,perlist,map_file
	     character*11 word    
	     character*1 wavetype
	     character*3 sym       
	     character*6 stnam(maxpath)
             character*16 evnam(maxpath)
c	     character*6 cdummy
             parameter (nmax=2000)
             real*4 per(50),grvel_R(50,maxpath),grvel_L(50,maxpath),vel(270000)
             real*4  stalat(maxpath),stalon(maxpath),evlat(maxpath),evlon(maxpath)
...
c	     data marg/3/,step/0.05/
             data  dlat_min/-90./,dlat_max/90.0/,dlon_min/-180.0/,dlon_max/180./
             data word/'PREDICTION_'/
             data const/2.0/,iconst/5/
C----------------------------------------------------------------------------------------

	     narg=iargc()
	     if(narg.ne.marg)STOP'USAGE: mhr_grvel_predict pathfile_name  map_name  perlist'
       Print*,'Produces group velocity curves from grv_maps for a given wavetype and a list of periods'
C--------------------initiation--------------------S
...
C--------search for paths-----------------S
	      print*, 'before calll path_searcher'
             call path_searcher(pathfile_name,ista,stalat,stalon,stnam,iev,evlat,evlon,evnam,npath)
...
c	     print *, 'Dist, step: ',dels, del, step
      	      ...
      	      call path(step,elat,elon,slat,slon,xalat,xalon,xray,npoints,i,IND)
...
C-----loop for period-----------------------------------------------------E
      	    ...
      	    do j=1,npath
       	      write(2,*) ista(j),iev(j),nper, ' ',stnam(j),' ',evnam(j), stalat(j), stalon(j), evlat(j), evlon(j)
       	      write(12,*) ista(j),iev(j),nper, ' ',stnam(j),' ',evnam(j), stalat(j), stalon(j), evlat(j), evlon(j)
...
C#####################################################################
            subroutine path_searcher(pathfile_name,ista,stalat,stalon,stnam,iev,evlat,evlon,evnam,npath)
            character*180 pathfile_name,curr_name
            character*6 stnam(100000)
            character*16 evnam(100000)
            real*4 stalat(100000),stalon(100000),evlat(100000),evlon(100000)
	    dimension ista(100000), iev(100000)
            logical lf
...
4           do j=1,1000000
            read(7,*,end=999) ista(j),iev(j),stnam(j),evnam(j),stalat(j),stalon(j),evlat(j),evlon(j)
		print *, ista(j),iev(j),' ',stnam(j),evnam(j),stalat(j),stalon(j),evlat(j),evlon(j)
            enddo
999         close(7)
	    npath=j-1
            return 
