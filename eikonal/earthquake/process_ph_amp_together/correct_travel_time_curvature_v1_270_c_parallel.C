...
*/
using namespace std;
// this corrects the curvature of both travel time and amp together
...
int  do_correct_curvature( char *infile, double period, double x0, double y0, int npts_x, int npts_y)
{
  /*if(na!=7)
    {
  ...
  radius=6371.1391285;
  pi=4.0*atan(1.0);
  double dx,dy,x1,y1,temp,temp1,temp2,temp3,temp4,temp5,lat_temp;
//  npts_x=176;
//  npts_y=126;
  //npts_x = atoi(arg[5]);
  //npts_y = atoi(arg[6]);
...
//  y0=25;
  ...
  sprintf(buff1,"%s.HD",infile);
  if((fin=fopen(buff1,"r"))==NULL)
    {
      cout<<buff1<<" not exist!!"<<endl;
      return -1;
    }
  sprintf(buff1,"%s_am.HD",infile);
  ...
  for(;;)
      ...
      if(i<0||j<0||i>npts_x||j>npts_y)
	{
      ...
      temp1=(tr_t[i][j+2]/-12.0+tr_t[i][j+1]*4.0/3.0+tr_t[i][j]*-5.0/2.0+tr_t[i][j-1]*4.0/3.0+tr_t[i][j-2]/-12.0)/dy_km[j]/dy_km[j];
      temp=temp+temp1;
      if(temp>0.005||temp<-0.005)
	{
      ...
      temp1=(tr_t[i][j+1]-tr_t[i][j-1])/2.0/dy_km[j];
      if(temp1==0)
	{
	  temp1=0.00001;
	}
      ...
      if(temp>0.5 || temp<1/6.5)
	{
      ...
      temp1=(amp[i][j+2]/-12.0+amp[i][j+1]*4.0/3.0+amp[i][j]*-5.0/2.0+amp[i][j-1]*4.0/3.0+amp[i][j-2]/-12.0)/dy_km[j]/dy_km[j];
