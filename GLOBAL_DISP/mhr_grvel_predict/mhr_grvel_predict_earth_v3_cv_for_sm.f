...
C----------------------------------------------------------------------------------------
C--pay attention to the formats of both input file and output file, the inaccurace of formats would results in segmentation fault
	     narg=iargc()
...
c	     write(*,*) "begin AZIDL ",slat,slon,elat,elon,i,j
...
c---- the output format
36          format(2i5,i3,a10,a2,a10,4f9.3)
...
C##################################################################### the N was 300000
     ...
     1      stalon,stnam,iev,evlat,evlon,evnam,npath)
	    ...
	    parameter ( N = 1200000)
            character*16 stnam(N)
            character*16 evnam(N)
            real*4 stalat(N),stalon(N),
     1                         evlat(N),evlon(N)
	    dimension ista(N), iev(N)
            logical lf
...
4           do j=1,2000000
     ...
     1               ," check"
            enddo
999         close(7)
c---- change this line to change the read in format, it should be the same format as the way you generate pathfile----- also, pay attention to the length of evnam and stnam
 2	  format(2i5,1x,a10,1x,a10,4(f10.3))
