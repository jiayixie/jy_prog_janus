...
//--------------------------------------
int compute_dispMineos(modeldef &model,vector<vector<double> > PREM,int Nprem, int Rsurflag,int Lsurflag,int ipara,const char *nodeid)
{ //execute mineos_bran, give a model, compute dispersion curves for L and/or R, store value in model.d.R/Ldisp.pvel/gvel
  ...
  sprintf(Minmodnm,"MineosInputMod_%s_%d.txt",nodeid,ipara);
...
//  cout<<"**** test Nmod="<<Nmod<<endl;// Nmod record the # of model layers
  ...
  sprintf(moddir,".");
  sprintf(modnm,"MineosInputMod_%s_%d",nodeid,ipara);
  if(Lsurflag>0){
     jcmp=2;
...
//---------------------------------------
int compute_kernel(paradef para,modeldef &model,vector<vector<vector<double> > > &kernel,vector<vector<double> > PREM,int Nprem, int Rsurflag,int Lsurflag,int isoflag,float depcri1,float depcri2,float qpcri,float qscri, const char *nodeid)
{// purterb the input para, then compute kernel
  ...
  if(Rsurflag>0 and Lsurflag==0){
	...
	compute_dispMineos(model,PREM,Nprem,1,0,0,nodeid);
	for(i=0;i<para.Rnpara;i++){
		tkp2.push_back(Rp0);tkg2.push_back(Rg0);
	}

	#pragma omp parallel for default(none) shared(nodeid,model,para,PREM,Nprem,depcri1,depcri2,qpcri,qscri,dp,tkp2,tkg2,tlkp2,tlkg2)  private (i,j,newpara,newmodel,tkp1,tkg1,tlkp1,tlkg1,DRpvel,DRgvel,DLpvel,DLgvel,ddp)
	for(i=0;i<para.Rnpara;i++){
	  ...
	  compute_dispMineos(newmodel,PREM,Nprem,1,0,i,nodeid);
	...
	tkp2.clear();tkg2.clear();
	compute_dispMineos(model,PREM,Nprem,1,1,0,nodeid);
	...
	for(i=0;i<para.Rnpara;i++){
	  ...
	  para2mod(newpara,model,newmodel);
	  updatemodelTibet(newmodel,depcri1,depcri2,qpcri,qscri);
	  compute_dispMineos(newmodel,PREM,Nprem,1,1,i,nodeid);
          compute_diff(newmodel.data.Rdisp,model.data.Rdisp,DRpvel,DRgvel);
	...
	for(i=0;i<para.Lnpara;i++){
	  ...
	  para2mod(newpara,model,newmodel);
          updatemodelTibet(newmodel,depcri1,depcri2,qpcri,qscri);
	  compute_dispMineos(newmodel,PREM,Nprem,1,1,i+para.Rnpara,nodeid);
	  compute_diff(newmodel.data.Rdisp,model.data.Rdisp,DRpvel,DRgvel);
	  compute_diff(newmodel.data.Ldisp,model.data.Ldisp,DLpvel,DLgvel);
	...
	exit(0);
  	tkp2.clear();tkg2.clear();
	compute_dispMineos(model,PREM,Nprem,0,1,0,nodeid);
	for(i=0;i<para.Rnpara;i++){
	  tkp2.push_back(Lp0);
	  tkg2.push_back(Lg0);
	}// for i Rnpara
	for(i=0;i<para.Lnpara;i++){
	  ...
	  para2mod(newpara,model,newmodel);
	  updatemodelTibet(newmodel,depcri1,depcri2,qpcri,qscri);
	  compute_dispMineos(newmodel,PREM,Nprem,0,1,i+para.Rnpara,nodeid);
	  compute_diff(newmodel.data.Ldisp,model.data.Ldisp,DLpvel,DLgvel);
...
//---------------------------------------------------------------
int read_kernel(paradef &para, modeldef &model,vector<vector<vector<double> > > &kernel, const char *fRker, const char *fLker,int &Rflag, int &Lflag,vector<vector<double> > PREM,int Nprem, int isoflag,const char *nodeid)
{
  //kernel: kernel[kRp[nP][nT],kRg[][],kLp[][],kLg[][]]
  ...
  compute_dispMineos(model,PREM,Nprem,Rflag,Lflag,0,nodeid);
...
int get_misfitMineos(modeldef &model,paradef &para,int Rflag, int Lflag,float depcri1,float depcri2,float qpcri,float qscri,vector<vector<double> > PREM,int Nprem,float inp, const char *nodeid)
