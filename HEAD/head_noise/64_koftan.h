...
#include <math.h>
//#include <malloc.h>
//#include "/home/zheng/progs/NOISE_CODA/HEAD_NOISE/gl_const.h"
//#include "/home/zheng/progs/NOISE_CODA/HEAD_NOISE/gl_const.h"
//#include "/home/zheng/progs/NOISE_CODA/HEAD_NOISE/mymacro.h"
...
#######################################################################*/
	...
	void Amp_Norm_Log (float *ain,float *aou,float *amax,int n);
	void Amp_Log_Norm (float *ampl,float *ampn,float amax,int n);

/*###################################################################
	Responce functions
###################################################################*/
	float FILT_2 (int n,float dt,float w);
	float FILT_4 (int n,float dt,float *c,float w);
	float FILT_1 (float w1,float w2,float w);
	float SKD_Amp_Resp
	(float Ts,float Tg,float Ds,float Dg,float s2,float w);
	float SKD_Pha_Resp
	(float Ts,float Tg,float Ds,float Dg,float s2,float w);
	float FILTN (int nfilt,long nftyp,float w);
	float FILT (float w);
	void ZER_POL (float *zer,int nz,float *pol,long np,float w,
			float *amp,float *pha);
	...
	void PZ_IRIS_MITPAN
	(float *zer,int nz,float *pol,long np,float camp);
	void P_Z_IRIS_MITP
	(float *zer,int nz,float *pol,long np);
	void Dis_Vel_Acc (char in,char out);
	void Dif_Resp (void);
	void Int_Resp (void);
	void INQ_SET_IRIS_RESP (int nsta,long jchn,long iy,long id);
	void I_S_IRIS_RESP (int nsta,long jchn,long iy,long id);
	void COOR_IRIS_STA (int nsta);
	void Change_dt_npoint (int numbc,long *n,float *point,float *dt);
...
###################################################################*/
	...
	void fl_ft_filter
 (float *spr,float *spi,float *crv,int npo,float *t,long unit,long pict);
	void TIME_FILTER (float *spr,float *spi,float *x,int pict);
