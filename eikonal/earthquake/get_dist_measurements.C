...
int main(int argn, char *arg[]) {
  clock_t t;
  int i,j,k,nper;
  char stanm[2000][10], ntnm[2000][10];
  double stalat[2000], stalon[2000],evlat,evlon,tevlat, tevlon;
...
//  double temp[4];
  ...
  for (i=0;;i++) {
    if (fscanf(ff1,"%s %lf %lf %s",&(stanm[i][0]),&(stalon[i]),&(stalat[i]),&(ntnm[i][0]))== EOF) break;
    }
  fclose(ff1);
  ...
  for (i=0;;i++) {
    if (fscanf(ff1,"%s %lf %lf", &(teve[0]),&(tevlon),&(tevlat))== EOF) break;
    if (strcmp(teve,evnm) != 0) continue;
    else {evlon = tevlon; evlat = tevlat;}
    }
  fclose(ff1);
  fprintf(stderr,"read file %s ok!\n",arg[2]);
  ...
  for (i=0;;i++) {
    if (fscanf(ff1,"%lf", &(pers[i]))== EOF) break;
    }
  nper = i;
  ...
  t = clock() - t;
  fprintf(stderr,"the time consumed: %g\n",((float)t)/CLOCKS_PER_SEC);
  return 1;
  }
...
double temp[4];
if (argn !=5) {
  fprintf(stderr,"please input lat1 lon1, lat2, lon2!!!\n");
  return 0;
  }
for (i=0;i<4;i++) {
  if (sscanf(arg[i+1],"%lf",&(temp[i]))==EOF) {
...
if (get_dist2(la1, lo1, la2, lo2, &dist, &az, &baz)==0) {
  fprintf(stderr,"cannot compute distance!!!");
  return 0;
  }
