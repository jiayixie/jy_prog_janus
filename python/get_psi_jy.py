...
def norm_range(psi,n):
	# this is used to regulate the range of psi; put psi to [0,T]
	T=2.*pi/n
	while (psi>T):
		psi=psi-T
	while(psi<0):
...
def readdata(indata,uncmag):
 ...
 for line in open(indata):
	...
	if(flagdata==0): # read information
		lon=float(l[0]);lat=float(l[1]);num=int(l[2])
		if(num==0):
			flagdata=0
			continue
		i=0
		...
		azi=float(l[0]);vel=float(l[1]);unc=float(l[2])*uncmag
		i=i+1
		if(unc>5): #err
...
def do_syn(M,U,id,azi):
	...
	if (id=='12psi'):
		...
		psi2t=psi2t/2.
		unc2t=unc2t/2.
		#psi1t=norm_range(psi1t,1)	
		#psi2t=norm_range(psi2t,2)	
...
if ( not os.path.exists(indata)):
	...
	sys.exit()
lonlst,latlst,numlst,datalst,azilst,unclst,Npoint=readdata(indata,uncmag)
...
for i in range(Npoint):
	...
	syn,var,varunc,varFA=do_syn(model,Munc,id,azi) # here I use azi instead of aziflt in purpose, keep the number of syn the same as the # of origional input data, otherwise, could use aziflt to get the reduced chi2
	"""
	#compute misfit ----------
	variance=sum((syn-data)**2)/num
	chi2=sum(((syn-data)/unc)**2) #chi2/num--> reduced chi2
	...
	for j in range(len(var)):
		if (j==0 or j%2!=0): #1st, or odd number, V0 or amplitude
			...
			str2="%s %5g"%(str2,var[j])
	#---write map file ---------------------
	#---lon, lat, azi_num, chi2, v0, dv0, A, dA, psi, dpsi, (in degree, CW from N)...
