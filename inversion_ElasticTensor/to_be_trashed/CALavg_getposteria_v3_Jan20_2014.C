...
// this version ,modified on Jan 20, 2014, output the infor for vsv,vsh,vpv,vph,eta,theta,phi; while previous version output vsv,vsh,and vsRA
#include<iostream>
#include<algorithm>
#include<vector>
#include<cmath>
...
using namespace std;
//----------------------------------------------------- 
	int para_avg_V(vector<paradef> &paralst, paradef &paraavg,vector<double> &parastd,vector<int> &idlst){
...
{//for each input model, write out the vsv~phi value from depmin(1st para) to depmax(2ed para) at depdx(3rd) interval.
  ...
  for (h=depmin;h<=depmax;h=h+depstep){
	...
	if (hlst[i]>h){
		...
		fprintf(out,"%8g %8g %8g %8g %8g  ",tvsv,tvsh,tvpv,tvph,teta,ttheta,tphi);break;}
    }
  }
...
// this version, we will calculate the uncertainty of anisotropy------	
  ...
  char fvpvnm[300],fvphnm[300],fetanm[300],fthetanm[300],fphinm[300];
  char faninm[300];
  //char fnmlst[7][300];
  ...
  while(1){
    ...
    if((para_avg_V(paraall,paraavg,parastd,idlst))==0){cout<<"### in para_avg, incorrect paralst.size()\n";exit(0);}
    printf("size=%d\n",idlst.size());
