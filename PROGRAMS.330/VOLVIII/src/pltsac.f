...
c      03 MAY 2013 - adjusted the -O command so that the time scale
c                    is properly placed
c                    NOTE - for nice plots with DOPLTSAC we need command 
c                    line to define time scale MIN and MAX
c      09 AUG 2013 - line 118 changed
c            call plotit(seis,nl,nh,npts,x0,y0,xlen,ylen,deg,user9,user5,
c         2          nzyear,nzjday,nzhour,nzmin,nzsec,fname,
c   to
c            call plotit(seis,nl,nh,npts,x0,y0,xlen,ylen,deg,user9,user5,
c         2          nzyear,nzjday,nzhour,nzmin,nzsec,fname(i),
c
c-----
        integer MXPTS
...
c-----
 ...
 1000   continue
        ...
        if(.not.doover .and. .not.doovershd)then
            ypos = y0
        else
            ypos = y0keep  - 0.5*ylen
        endif
        if(dotmbt)then
            if(doscbt)then
