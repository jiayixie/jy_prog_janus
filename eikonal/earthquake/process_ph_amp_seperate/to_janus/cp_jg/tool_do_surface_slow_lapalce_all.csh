...
#foreach per ( 8 10 12 14 16 18 20 22 24 25 26 28  30 32 34 35 36 38 40 42 44 45 46 48 )
	...
	set j = `echo $i | awk '{print $1+1}'`
        ...
        set dis2 = ${distlst[$j]}
	echo $dis1 $dis2
	set outdir = $per"sec_"$snr_cri"snr_${dis1}_${dis2}dist"
	mkdir $outdir
	cd $outdir
	set dirph = . #../$per"sec_"$snr_cri"snr_${dis1}_${dis2}dist" # the dir that contains *ph.txt* files
	##############change here ###########
	ls $dirph/*.ph.txt_v2 | cut -d/ -f2 | cut -d. -f1 > event_lst_ph
	#####################################
	...
	/home/jiayi/progs/jy/eikonal/earthquake/travel_time_to_slow_map_v4_v3_v2to10_270_noampHD_jy_g0.4_4direction_sepDir $evlstph  $per $dist_cri $grid  $dirph
	echo 'begin to run slow map to iso map..................'
	/home/jiayi/progs/jy/eikonal/earthquake/slow_maps_to_iso_map_ani_data_v5_n50_270_noamp_25p_g0.4_sepDir $evlstph  0 360 18 $per"s_iso_ani_v1" $grid $dirph
