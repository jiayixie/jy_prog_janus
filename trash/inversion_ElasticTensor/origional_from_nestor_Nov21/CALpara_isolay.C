...
//-----------------------------------------------------	 
	int checkParaModel(paradef para, modeldef model){
	  ...
	  for(i=0;i<model.ngroup;i++){
		...
		if(para.space1[j][2]<-1. and (p6-1)*(p6-2)*(p6-6)*(p6-7)*(p6-10)*(p6-11)==0 ){//this theta or phi or AZcos or AZsin will be scaled according to other values(the 1st value in that group) during the para2mod process
			//also, vsv, vsh can be sacled according to the 1st value in that group; to set the same amount of VsRA
			if(p5<1){
		...
		if(p0==0){ //value
		  ...
		  if(p6==1){
			  ...
			  if(para.space1[i][2]<-1.)//use the RAvp, eta, RAvs relation
			  {
				...
				factor=(outmodel.groups[ng].vshvalue[nv]-outmodel.groups[ng].vsvvalue[nv])/(outmodel.groups[ng].vshvalue[nv]+outmodel.groups[ng].vsvvalue[nv])*0.5; //0.25*RAvs
...
//----------------------------------------------------- 
	int para_avg(vector<paradef> &paralst, paradef &paraavg,vector<double> &parastd,vector<double> &LoveRAparastd, vector<vector<double> > &LoveAZparastd,vector<int> &idlst){
