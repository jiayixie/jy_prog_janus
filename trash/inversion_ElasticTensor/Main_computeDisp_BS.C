...
sprintf(tmpstr,"if [ ! -d %s/binmod ]; then mkdir %s/binmod; fi",dirlay,dirlay);
  ...
  while(1){
    ...
    */
    ...
    float theta;
    for(theta=0;theta<10;theta=theta+30){
    //for(theta=60.;theta<70;theta=theta+30.){
    ...
    /*modelnew.groups[1].vshvalue[0]=modelnew.groups[0].vshvalue[1]*1.01;
    modelnew.groups[1].vshvalue[1]=modelnew.groups[1].vshvalue[1]*1.01;
    modelnew.groups[1].vshvalue[2]=modelnew.groups[2].vshvalue[1]*1.01;
    modelnew.groups[1].thetavalue[0]=theta;
//76.2203 81.6582
    //modelnew.groups[1].thetavalue[0]=90.;
    modelnew.groups[1].phivalue[0]=0.;
   ...
   */
    ...
    */
    ...
    write_ASC(modelnew,paranew,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,AziampRsurflag,AziampLsurflag,AziphiRsurflag,AziphiLsurflag);
    ...
    compute_dispMineos(modelnew,PREM,Nprem,Rsurflag,Lsurflag,0);
    sprintf(modnm1,"%s/AnimodM_%.0f_%s_%.1f_%.1f.txt_TI",dirlay,theta,nodeid,lon,lat);
    sprintf(fRdispnm,"%s/RdispM_%.0f_%s_%.1f_%.1f.txt_TI",dirlay,theta,nodeid,lon,lat);
    sprintf(fLdispnm,"%s/LdispM_%.0f_%s_%.1f_%.1f.txt_TI",dirlay,theta,nodeid,lon,lat);
    write_ASC(modelnew,paranew,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,0,0,0,0);
    //*/
