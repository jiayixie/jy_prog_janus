...
//--------------------------
vector<int> para_avg_multiple_gp(vector<int> idphiSlst,vector<int> idphilst,vector<int> idphiMlst, vector<paradef> &paralst, vector<paradef> &parabestlst, vector<paradef> &paraavglst, vector<paradef> &parastdlst, vector<vector<int> > &idlstlst, int flag, double &pkC, vector<int> AZcosidlstS,vector<int> AZcosidlst, vector<int> AZcosidlstM,modeldef model, int flagupdaterho, paradef para0){
  ...
  // on Jul 7, 2015, added the idphiSlst, and AZcosidlstS, i.e., phi and AZ information for the sediment (gp0). The current version of this function is not good, the number of group is constrained (3 in this modified version), should make it more flexible, suitable to any number of groups
  //
  int i,j,k,n,size,idmin,NgpS,Ngp,NgpM,igp,pflag,Ngood,idphiS,idphi,idphiM;
  vector<int> indexflaglstS,indexflaglst,indexflaglstM,idlst,idlstnew,idminlst;
  //vector<int> idlstnewtest;
  vector<double> philstS,philst,philstM;  
  double mismin,pkM,pk,pkS,tv,T;  
  ...
  */
  ...
  //---sediment ----
  if(AZcosidlstS.size()>0){// if this sed groups is AZcos}{
  indexflaglstS=convert_AZpara(paralst, AZcosidlstS,idlst,Ngood,Ngp,100.); // here I set the sepangcri=100. to eliminate group seperation in the sediment
  }
  else{//a  TTI/TI/iso model
  if(idphiSlst.size()>0){
  for(n=0;n<idphiSlst.size();n++){
    philstS.clear();
    idphiS=idphiSlst[n];
    for(i=0;i<Ngood;i++){
	k=idlst[i];
	philstS.push_back(paralst[k].parameter[idphiS]);
    }//for i<Ngood	
    NgpS=0;
    seperate_gp(philstS,NgpS,pkS,indexflaglstS,15.,T);
    if(NgpS==1){//only one group
	for(i=0;i<Ngood;i++){
		k=idlst[i];
		tv=paralst[k].parameter[idphiS];
		paralst[k].parameter[idphiS]=convert(tv,pkS,T);
	}
    }//if NgpS==1
    else{// two groups, one with center around pkS, the other around pkS+T*0.5 (pk+90)
	for(i=0;i<Ngood;i++){
		if(indexflaglstS[i]==1){
			tv=paralst[idlst[i]].parameter[idphiS];
			paralst[idlst[i]].parameter[idphiS]=convert(tv,pkS,T);
		}
		else if (indexflaglstS[i]==2){
			tv=paralst[idlst[i]].parameter[idphiS];
			paralst[idlst[i]].parameter[idphiS]=convert(tv,pkS+0.5*T,T);
		}
	}// for i<Ngood

    }//else two groups
  }//for n<idphiSlst.size()
  }//if idphiSlst
  else{// else idphiSlst.size>0. No phi in this group
    indexflaglstS.clear();NgpS=1;for(i=0;i<Ngood;i++){indexflaglstS.push_back(1);}}
  }//else AZcosidlstS
...
//---to be modified----here, the multiple group in sediment is not added
  for(i=0;i<Ngp;i++){
	for(j=0;j<NgpM;j++){
  		idlstnew.clear();
