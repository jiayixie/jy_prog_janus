...
//  abort();  
  ...
  if(fscanf(file1,"%s %lf %lf",&sta1,&sta1_lon,&sta1_lat)==EOF)
	 ...
	 { sta1_lon = sta1_lon + 360.; }
      ...
      sprintf(buff1,"%s.%g.input.c.txt_v2.HD",sta1,period);
	 ...
	 continue;
      ...
      sprintf(buff1,"%s.%g.input.c.txt_v2.HD_0.2",sta1,period);
...
//      sprintf(buff1,"slow_azi_%s.%s.txt.HD.2.v2",sta1,pflag);
      sprintf(buff1,"slow_azi_%s.%g.txt.HD.2.v2",sta1,period);
      fout=fopen(buff1,"w");
      for(i=0;i<npts_x;i++)
	{
	  for(j=0;j<npts_y;j++)
...
//      sprintf(buff1,"travel_time_%s.%s.txt_v1",sta1,pflag);
      sprintf(buff1,"%s.%g.input.c.txt_v2",sta1,period);
      if((fin3=fopen(buff1,"r"))==NULL)
	{
	  cout<<buff1<<" not exist!!"<<endl;
	  return 1;
