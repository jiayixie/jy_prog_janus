...
int write_ASCAZdisp_single(dispdef AMPdisp, dispdef PHIdisp, FILE *outdisp,dispdef RAdisp){
  ...
  if(AMPdisp.npper>PHIdisp.npper){
  	for(i=0;i<AMPdisp.npper;i++){
		if(i<PHIdisp.npper)
			fprintf(outdisp,"amp %5g %10g %10g %10g  phi %5g %10g %10g %10g\n",AMPdisp.pper[i],AMPdisp.pvelo[i],AMPdisp.pvel[i],AMPdisp.unpvelo[i],PHIdisp.pper[i],PHIdisp.pvelo[i],PHIdisp.pvel[i],PHIdisp.unpvelo[i]);
		else{
			fprintf(outdisp,"amp %5g %10g %10g %10g  phi %5g %10g %10g %10g\n",AMPdisp.pper[i],AMPdisp.pvelo[i],AMPdisp.pvel[i],AMPdisp.unpvelo[i],0.,0.,0.,0.);
...
int write_ASC(modeldef model,paradef para, char *namemod, char *Rnamedisp,char *Lnamedisp,char *AZRnamedisp, char *AZLnamedisp, int Rsurflag, int Lsurflag, int RAZampflag, int LAZampflag,int RAZphiflag, int LAZphiflag)
{
 ...
 fprintf(outmod,"L %8g  RL %8g %8g %8g LL %8g %8g %8g  \nmisfit %8g Rm %8g %8g %8g Lm %8g %8g %8g\n",model.data.L,model.data.Rdisp.L,model.data.AziampRdisp.L,model.data.AziphiRdisp.L,model.data.Ldisp.L, model.data.AziampLdisp.L,model.data.AziphiLdisp.L,  model.data.misfit, model.data.Rdisp.misfit, model.data.AziampRdisp.misfit, model.data.AziphiRdisp.misfit,model.data.Ldisp.misfit, model.data.AziampLdisp.misfit,model.data.AziphiLdisp.misfit);
  ...
  fprintf(outmod,"Npara %d  Ngroup %d Nlayer %d\n",para.npara,model.ngroup,model.laym0.nlayer);
  for(i=0;i<para.npara;i++)
        fprintf(outmod,"%10g ",para.parameter[i]);
  fprintf(outmod,"\n");
  fprintf(outmod,"%10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s\n","depth","vsv","vsh","vpv","vph","eta","theta","phi","qs","qp","rho","vpvs");
  for(i=0;i<model.laym0.nlayer;i++){
  	fprintf(outmod,"%10g %10g %10g %10g %10g %10g %10g %10g %10g %10g %10g %10g\n",h,model.laym0.vsv[i],model.laym0.vsh[i],model.laym0.vpv[i],model.laym0.vph[i],model.laym0.eta[i],model.laym0.theta[i],model.laym0.phi[i],model.laym0.qs[i],model.laym0.qp[i],model.laym0.rho[i],model.laym0.vpvs[i]);
	h=h+model.laym0.thick[i];
  }
  /*
