...
#endif
#define NEVTMAX 8000
// this version combine the eqk (vel_evnm_v?.txt) and the noise (slow_azi_stnm*) together to genrate the iso ani results
using namespace std;
...
//  y0=25;
  ...
  for(ii=0;ii<neveqk;ii++){
	...
	sprintf(buff1,"%s/vel_%s_v4.txt",direqk,event_name);
      if((fin=fopen(buff1,"r"))==NULL)
       ...
       nsta++;
       cv1=0;
       if (fmod(cv2,50)==1) printf("now read %d th %s\n",cv2,buff1);
       for(;;)
       {
         if (fgets(tstr,300,fin) == NULL) break;
	 if(sscanf(tstr,"%lf %lf %lf %lf %lf %lf %lf",&lon,&lat,&temp,&temp2,&trash1,&trash2,&trash3)!=7)continue;
 	 temp=1./temp; //vel->slowness
  ...
  printf("read ok! read in %d events+station, %d of them has vel+slow_azi file\n",neveqk+nevnoise,nsta);
  nsta=50;
  double w2;
