...
/* Changes:
	...
	11 JUL 2010 - added the ~ command which then creates a screen dump 
        ...
        10 OCT 2012 - fixed annoying bug that caused the pick to be undone when
                      a command was made outside the trace window
	17 MAY 2013 - permit BUTTON 4 and 5 from mouse scroll to adjust 
			amplitude as surrogate for '*' and '/' respectively
*/
#include	<stdio.h>
#include	"gsac.h"
...
char ostr[9];
	...
	for(i=0 ; i < nmp ; i++){
		...
		if(inside(cx,cy,lx,ly,hx,hy)){
			/* 10 OCT 2012 this block of code was just above the previous statement */
                	if(cmd ==  MENU_P_UNDO ){
				if(last_h > 0){
					pickundo = YES;
