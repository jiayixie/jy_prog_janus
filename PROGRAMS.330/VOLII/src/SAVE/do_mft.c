		...
		if(strncmp(tcp[i],"-11MIN",6) ==0){
			...
			switch(tcp[i][1]){
				...
				case '?':
					...
					fprintf(stderr,"\n");
					fprintf(stderr,"  -G              (default no) The default dispersion file name is of the form \n");
					...
					fprintf(stderr,"  -T              (default no)  run script MFTDOOVERLAY\n");
					return (0);;
				default:
					break;
			}
...
int Pick_file(int npage, int aargc, int *curpage)
{
	...
	mgwrtxt(0.2,6.8,
		...
		1,   1);
	mgwrtxt(0.2,6.6,
	"  Stnm    Cmpnm          Npts    Bytes    First Sample Time          Dist    Az Proc",
		0,   4);
	page_entry = 2;
	p = Start;
	while(p != End){
