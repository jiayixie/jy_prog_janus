...
vector<int> seperate_gp(vector<double> &vlst,int &Ngp){
  ...
  //
  //#############PARAMETER
  float distcri=15;//if |peak-avg|>distcri, then we think there are multiple groups of phi value
  ...
  pk=(k+0.5)*dv;//the mid value of that bin;
...
//--------------------------
int para_avg_multiple_gp(int idphi,vector<paradef> &paralst, paradef &parabest, vector<paradef> &paraavglst, vector<paradef> &parastdlst, vector<vector<int> > &idlstlst){
  int i,j,k,size,idmin,Ngp,igp,pflag,Ngood;
  vector<int> indexflaglst,idlst,idlstnew;
  vector<double> philst;  
  double mismin;  
  ...
  */
  ...
  //--get the smallest misfit and get the index lst for model within misfit criteria--
  mismin=1e10;
  for(i=0;i<size;i++){
  ...
  for(i=0;i<Ngp;i++){
	...
	for(j=0;j<Ngood;j++){
		if(indexflaglst[j]==i+1){//id belongs to this group
			idlstnew.push_back(idlst[j]);
		}
  	}//for j<Ngood
	paraavg.parameter=compute_paraavg(paralst,idlstnew,parastd.parameter,1);
	//printf("get the paraavg.parameter\n");//--check--
	//paraavg.LoveRAparameter=compute_paraavg(paralst,idlstnew,parastd.LoveRAparameter,2);
	//paraavg.LoveAZparameter=compute_paraavgAZ(paralst,idlstnew,LoveAZparastd,3);
