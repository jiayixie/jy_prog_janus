...
// this version, we will calculate the uncertainty of anisotropy------	
  ...
  //---end of reading information
    ...
    modeldef ttmodel,RAmodel,tmodel,modelref;
    paradef tpara,RApara,pararef;
    vector<modeldef> modelreflst;
    vector<paradef> parareflst;
    ///*
    for(int ig=0;ig<idlstlst.size();ig++){
  	//--the average Ppara 
  	paraP.parameter=paraavglst[ig].parameter;
  	paraP.LoveRAparameter=paraavglst[ig].LoveRAparameter;
  	paraP.LoveAZparameter=paraavglst[ig].LoveAZparameter;
 	ttmodel=modelP;
	printf("para2mod\n");
	...
	pararef=para1;
	modelref=model0;
	parareflst.push_back(pararef);
	modelreflst.push_back(modelref);

 	printf("@@@ check, do V kernel ====\n");
    	//---obtain V kernel ---
	sprintf(kernelnmR,"%s/VkernelRp1ani_avg_gp%d_%s_%.1f_%.1f.txt",dirlay,ig,nodeid,lon,lat);
	sprintf(kernelnmL,"%s/VkernelLp1ani_avg_gp%d_%s_%.1f_%.1f.txt",dirlay,ig,nodeid,lon,lat);
	...
	printf("@@@ check, do L kernel ====\n");
    	//---obtain Love kernel ---
	sprintf(kernelnmR,"%s/LkernelRp1ani_avg_gp%d_%s_%.1f_%.1f.txt",dirlay,ig,nodeid,lon,lat);
	sprintf(kernelnmL,"%s/LkernelLp1ani_avg_gp%d_%s_%.1f_%.1f.txt",dirlay,ig,nodeid,lon,lat);
	...
	for(int j=0;j<paraall.size();j++){
		...
		if(ig==0){
		  ...
		  /*
	          sprintf(modnm1,"%s/AnimodT_%d_%s_%.1f_%.1f.txt_phigp%d",dirlay,j,nodeid,lon,lat,ig);
                  sprintf(fRdispnm,"%s/RdispT_%d_%s_%.1f_%.1f.txt_phigp%d",dirlay,j,nodeid,lon,lat,ig);
                  sprintf(fLdispnm,"%s/LdispT_%d_%s_%.1f_%.1f.txt_phigp%d",dirlay,j,nodeid,lon,lat,ig);
                  sprintf(fAZRdispnm,"%s/AZRdispT_%d_%s_%.1f_%.1f.txt_phigp%d",dirlay,j,nodeid,lon,lat,ig);
                  sprintf(fAZLdispnm,"%s/AZLdispT_%d_%s_%.1f_%.1f.txt_phigp%d",dirlay,j,nodeid,lon,lat,ig);
