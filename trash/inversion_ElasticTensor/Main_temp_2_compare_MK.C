...
// read in a model, compute its TI part, compute kernel; then perturb it, compute the Kernel disp and Mineos disp
...
*/
	...
	fscanf(inpo,"%g",&tempP);
	if((i-0)%7==0 and i<10 )
		paraP.parameter[i]=tempP*1.05;
	//if(tempP<10 and i<10 )
	//	paraP.parameter[i]=tempP*1.1;
	else
		paraP.parameter[i]=tempP;
	printf("i=%d,paraP=%g\n",i,paraP.parameter[i]);
    }
    ...
    //---
		...
		*/
	    ...
	    write_ASC(modelP,paraP,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,AziampRsurflag, AziampLsurflag,AziphiRsurflag, AziphiLsurflag);
            ...
            sprintf(fAZRdispnm,"%s/AZRdispM_%.0f_%s_%.1f_%.1f.txt_phigp%d",dirlay,theta,nodeid,lon,lat,ig);
            sprintf(fAZLdispnm,"%s/AZLdispM_%.0f_%s_%.1f_%.1f.txt_phigp%d",dirlay,theta,nodeid,lon,lat,ig);

	   get_misfitMineosRA(modelP,paraP,PREM,Nprem,Rsurflag,Lsurflag,flagupdaterho);
	   write_ASC(modelP,paraP,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,0,0,0, 0);
