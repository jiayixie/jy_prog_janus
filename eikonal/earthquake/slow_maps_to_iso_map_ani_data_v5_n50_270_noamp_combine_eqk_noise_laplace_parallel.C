...
#endif
#define NEVTMAX 8000
// this version combines the eqk (slow_azi* and *laplace*) and the noise (slow_azi*) to generate iso and ani result
// THIS CODE HAD PROBLEM, STILL UNDER DEBUGGING 
using namespace std;
...
int main(int na, char *arg[])
{
  ...
  FILE *ff,*fin,*fin2,*fout,*file1,*file_iso,*file_ani;
  int i,j,k;
  int npts_x,npts_y;
  char buff1[300],buff2[300],sta1[10],name_iso[100],name_ani[100],name_ani_n[100],direqk[200],dirnoise[200];
...
//  npts_y=126;
  ...
  double slow[npts_x][npts_y][NEVTMAX],slow_weight[npts_x][npts_y][NEVTMAX],laplace[npts_x][npts_y];
...
//  y0=25;
  ...
  printf("from list, read in %d eqk, and %d station\n",neveqk,nevnoise);
  // read in the slow_azi and laplace file of earthquake
  cv2 = 0;
  #pragma omp parallel for default(none)  private (cv1,ii,buff2,event_name,buff1,fin2,fin,i,j,tstr,lon,lat,temp,temp2,trash1,trash2,trash3,trash4,trash5,laplace) shared (cv2,events_lst,direqk,per,npts_x,npts_y,nsta,x1,x0,y1,y0,dx,dy,slow,n,neqk,slow_weight,azi,neveqk)
  for(ii=0;ii<neveqk;ii++){
	//printf("threads=%d\n",omp_get_num_threads());//--check---
	if(fmod(ii,50)==0){printf("work on the %dth eqk\n",ii);}
	cv2=cv2+1;
	sprintf(event_name,"%s\0",events_lst[ii]);
	//check the availability of both laplace and slow_azi files---
	sprintf(buff2,"%s/%s.%d.input.c.txt_am_laplace.txt.HD",direqk,event_name,per);

	sprintf(buff1,"%s/slow_azi_%s.%d.txt.HD.2.v2",direqk,event_name,per);
	//printf("hey read eqk %d\t %s %s\n",ii,buff1,buff2);
	if((fin2=fopen(buff2,"r"))==NULL)continue;
	if((fin=fopen(buff1,"r"))==NULL)continue;

	//-----------------
       if (fmod(cv2,50)==1) printf("now read %d th eqk %s\n",cv2,buff1);
	for(i=0;i<npts_x;i++){
		for(j=0;j<npts_y;j++){
			laplace[i][j]=9999.;
		}
	...
	for(;;){
		if (fgets(tstr,300,fin2) == NULL) break;
		if(sscanf(tstr,"%lf %lf %lf",&lon,&lat,&temp)!=3)continue;
		//printf("read laplace, %g %g %g\n",lon,lat,temp);//--check--
		if(lon<0)lon=lon+360.;
         	if(lon>x1+0.01||lon<x0-0.01|lat>y1+0.01||lat<y0-0.01)continue;
		i=int((lon-x0)/dx+0.1);
		j=int((lat-y0)/dy+0.1);
		//if(temp>0.01 or temp<-0.01)continue; // constraint
		laplace[i][j]=temp;	
	}
	fclose(fin2);
       nsta++;
	 ...
	 //printf("read slow_azi, %g %g %g\n",lon,lat,temp);//--check--
	 if(lon<0)lon=lon+360.;
         if(lon>x1+0.01||lon<x0-0.01|lat>y1+0.01||lat<y0-0.01)
           continue;
         i=int((lon-x0)/dx+0.1);
         j=int((lat-y0)/dy+0.1);
         if(temp<0.5&&temp>0.1&&laplace[i][j]<0.01&&laplace[i][j]>-0.01)
           {
             slow[i][j][n[i][j]]=sqrt(temp*temp-laplace[i][j]);//slowness+laplace
             azi[i][j][n[i][j]]=temp2;
  ...
  #pragma omp parallel for default(none) private (ii,event_name,fin,buff1,cv1,tstr,lon,lat,temp,temp2,trash1,trash2,trash3,trash4,trash5,i,j,x0,x1,dx,y0,y1,dy) shared (neveqk,nevnoise,events_lst,dirnoise,cv2,nsta,n,slow_weight,slow,azi)
  for(ii=neveqk;ii<neveqk+nevnoise;ii++)
    {
      //printf("threads=%d\n",omp_get_num_threads());//--check---
      if(fmod(ii,50)==0){printf("work on the %dth noise\n",ii);}
      cv2 = cv2+1;
      sprintf(event_name,"%s\0",events_lst[ii]);
      //nsta++;
      sprintf(buff1,"%s/slow_azi_%s.phase.c.txt.HD.2.v2",dirnoise,event_name);
	 ...
	 continue;
       ...
       cv1=0;
       nsta++;
       if (fmod(cv2,50)==1) printf("now read %d th noise %s\n",cv2,buff1);
       for(;;)
       {
	 if (fgets(tstr,300,fin) == NULL) break;
         if (sscanf(tstr,"%lf %lf %lf %lf %lf %lf %lf %lf %lf",&lon,&lat,&temp,&temp2,&trash1,&trash2,&trash3,&trash4,&trash5) != 9) continue;
  ...
  printf("read ok! read in %d events+station, %d of them has laplace,slow_azi file\n",neveqk+nevnoise,nsta);
  nsta=50;
  double w2;
