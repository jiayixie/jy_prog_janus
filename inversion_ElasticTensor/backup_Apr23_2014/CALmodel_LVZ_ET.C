	  ...
	  while(getline(mff,line))
		{
		  ...
		  if((model.groups[iid].flagcpttype==2 or model.groups[iid].flagcpttype==4) and model.groups[iid].flag!=1){// Lovepara/kernel, only for layered model
		 	printf("##### readmod, wrong flagcpttype vs. flag!\n");
			exit(0);
		  }
