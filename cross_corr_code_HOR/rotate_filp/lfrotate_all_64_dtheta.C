...
// this version, rotate data to perturbed T-R component, the angle is defined by dtheta
using namespace std;
...
{ 
  ...
  if(na!=4)
    {
      cout<<"usage:lfroate stalist path "<<na<<endl;
      return 0;
    }
  FILE *f1,*f2,*f3,*ft;
  int N=3000; //################### change this limit of station number !!!
  double PI;  
  PI=atan(1.0)*4;
  double temp1,temp2,dtheta;
  double cos1,cos2,sin1,sin2;
  char name[N][10],ftname[300];
  ...
  ft=fopen("file_with_cmpazProblem.lst","w");
  f1=fopen(arg[1],"r");
  dtheta=atof(arg[3]);
  int i,j,k,ii,jj;
  for(i=0;i<N;i++)
...
//	  abort();
	  if(temp1>0.1||temp2>0.1)
	    {
	      cout<<"cmpaz incompatible!!"<<" "<<name_EE<<endl;
	      fprintf(ft,"j=%5d k=%5d %10s %10s %s\n",j,k,name[j],name[k],name_EE);
	      continue;
	    }
	  temp1=(shd_cor_NN.az-shd_cor_NN.cmpaz+dtheta)/180.0*PI;
	  cos1=cos(temp1);
	  sin1=sin(temp1);
	  temp2=(shd_cor_NN.baz-shd_cor_NN.user9+dtheta)/180.0*PI;
	  cos2=cos(temp2);
	  sin2=sin(temp2);
