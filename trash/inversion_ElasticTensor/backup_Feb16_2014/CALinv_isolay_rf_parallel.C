...
//  oldmisfit=refmodel.data.misfit;
  ...
  //num_threads(Njump)
  #pragma omp parallel for default(none) shared(paralst,flagupdaterho,iloop,start,k1,k2,Njump,iiter,iaccp,dtime,start2,accpcri,iitercri,misfitcri,id,key,PosAni,isoflag,inpamp,inpphi,refpara,refmodel,Rmonoc,Lmonoc,Rvgrad,Lvgrad,Vposani,Rvmono,Lvmono,Lsurflag,Rsurflag,AziampRflag,AziampLflag,AziphiRflag,AziphiLflag,Vkernel,Lkernel,outbin,outbinRA) private(tpara,para2,para1,ttmodel,tmodel,oldLL,oldRL,oldL,countitt,countacc,ibad,tparalst,tmodellst,tflag,newL,newRL,newLL,newmisfit,now,prob,oldmisfit,prandom) 
	...
	*/
	...
	for(countitt=0;countitt<iitercri;countitt++){
		...
		*/
		...
		//test-- tmodellst.push_back(ttmodel);
		countacc++;		
