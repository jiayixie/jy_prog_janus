...
#include "/home/jiayi/progs/jy/HEAD/head_noise/64_sac_db_jy.h"
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
/*--------------------------------------------------------------------------*/
 int do_cor( SAC_DB *sdb, int lag, char *ch )
/*--------------------------------------------------------------------------*/
{
...
//printf("ok here2 \n");
	...
	len = shdamp1.npts;
     ...
     for( jsta2 = (jsta1+1); jsta2 < sdb->nst; jsta2++ ) {
	    ...
	    sprintf(filename_tp0, "COR_%s/COR_%s.%s_%s.%s.%s.SAC",ch,sdb->st[jsta1].ntnm,sdb->st[jsta1].name, sdb->st[jsta2].ntnm,sdb->st[jsta2].name,ch);

 
       if( access(filename_tp0, F_OK) == 0) {  //signal exists, skip
	  printf("%s has existed already\n",filename_tp0);
	  goto loop2;
        }

//printf("ok here3 \n");
            sprintf(filename_tp, "COR_%s/COR_%s.%s_%s.%s.%s.SAC",ch,sdb->st[jsta2].ntnm,sdb->st[jsta2].name, sdb->st[jsta1].ntnm,sdb->st[jsta1].name,ch);




       if( access(filename_tp, F_OK) == 0) { //reversed signal exists, reverse the signal and flip the name into tp0
          printf("%s has existed already\n",filename_tp);
...
//printf("ok here 5 \n");
	    ...
	    sprintf(filename, "COR_%s/COR_%s.%s_%s.%s.%s.SAC.prelim",ch,sdb->st[jsta1].ntnm,sdb->st[jsta1].name, sdb->st[jsta2].ntnm,sdb->st[jsta2].name,ch);
	    ...
	    // if file doesn't exist, use one of the current headers
...
/*--------------------------------------------------------------------------*/
{
  FILE *ff;
  int ns1 = 0, ns2 = 0,lag;
  char str[600], filename[200],channel[10];

  if ( na != 3 )
    {
      fprintf(stderr,"usage: corr_filtered_5000 1]lag 2]channel\n");
      exit(1);
    }

  sscanf(arg[1],"%d", &lag );
  sscanf(arg[2],"%s",channel);
...
fprintf(stderr, "read info fine\n");
  ...
  for ( ns2 = 1; ns2 < sdb.nst; ns2++ ) for ( ns1 = 0; ns1 < ns2; ns1++ ) {
    sprintf(filename, "COR_%s/COR_%s.%s_%s.%s.%s.SAC.prelim", channel,sdb.st[ns1].ntnm,sdb.st[ns1].name, sdb.st[ns2].ntnm,sdb.st[ns2].name,channel);
//    sprintf(str, "mv COR_%s/COR_%s.%s_%s.%s.SAC.prelim COR/COR_%s.%s_%s.%s.SAC",
//     sdb.st[ns1].ntnm, sdb.st[ns1].name, sdb.st[ns2].ntnm,sdb.st[ns2].name,sdb.st[ns1].ntnm, sdb.st[ns1].name, sdb.st[ns2].ntnm,sdb.st[ns2].name);
    sprintf(str, "mv %s COR_%s/COR_%s.%s_%s.%s.%s.SAC",filename,channel,sdb.st[ns1].ntnm, sdb.st[ns1].name, sdb.st[ns2].ntnm,sdb.st[ns2].name,channel);
