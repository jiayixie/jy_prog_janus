...
//CALforward_Mineos_readK_parallel_BS_newV2L.C  Main_computeDisp_newV2L.C
//CALpara_isolay_BS_newV2L.C
//
...
vector<vector<vector<double> > > debugLkernel1,debugLkernel2,debugLkernel3;
...
sprintf(tmpstr,"if [ ! -d %s/binmod ]; then mkdir %s/binmod; fi",dirlay,dirlay);
  ...
  while(1){
    ...
    */
    ...
    float theta;
    //for(theta=0.;theta<4;theta=theta+10.){
    for(theta=0.;theta<92;theta=theta+10.){
...
//76.2203 81.6582
    ...
    */
    ...
    //---compute the ISOTROPIC TI part with Mineos ----
    printf("rho=%g\n",modelnew.groups[1].rhovalue[0]);
    for(i=0;i<paranew.npara;i++){printf("Lpara%d=%f\n",i,paranew.LoveRAparameter[i]);}
    Lovepara2Vpara(paranew,modelnew);
    modeltemp=modelnew;
    for(i=0;i<paranew.npara;i++){
    	int p6=(int)paranew.para0[i][6];
   	if(p6==2 or p6==4){paranew.parameter[i]=paranew.parameter[i-1];}
	if(p6==5){paranew.parameter[i]=1.;}
    }
    para2mod(paranew,modeltemp,modelnew);
    updatemodel(modelnew,flagupdaterho);
    compute_dispMineos(modelnew,PREM,Nprem,Rsurflag,Lsurflag,0);
    sprintf(fRdispnm,"%s/RdispMIso_%.0f_%s_%.1f_%.1f.txt",theta,dirlay,nodeid,lon,lat);
    sprintf(fLdispnm,"%s/LdispMIso_%.0f_%s_%.1f_%.1f.txt",theta,dirlay,nodeid,lon,lat);
    write_ASC(modelnew,paranew,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,0,0,0,0);	
