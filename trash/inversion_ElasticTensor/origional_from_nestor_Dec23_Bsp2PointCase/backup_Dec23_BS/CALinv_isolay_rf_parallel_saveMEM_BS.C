...
//  oldmisfit=refmodel.data.misfit;
  ...
  //num_threads(Njump)
  #pragma omp parallel for default(none) shared(flagupdaterho,iloop,start,k1,k2,Njump,iiter,iaccp,dtime,start2,accpcri,iitercri,misfitcri,id,key,PosAni,isoflag,inpamp,inpphi,refparaBS,refmodelBS,refpara,refmodel,Rmonoc,Lmonoc,Rvgrad,Lvgrad,Vposani,Rvmono,Lvmono,Lsurflag,Rsurflag,AziampRflag,AziampLflag,AziphiRflag,AziphiLflag,Vkernel,Lkernel,outbin,outbinRA) private(tpara,para2,para1,ttmodel,tmodel,oldLL,oldRL,oldL,countitt,countacc,ibad,tparalst,tflag,newL,newRL,newLL,newmisfit,now,prob,oldmisfit,prandom) 
  for(int i=0;i<Njump;i++){
	...
	paradef RApara,para1BS,para2BS,tparaBS;
	...
	*/
	...
	for(countitt=0;countitt<iitercri;countitt++){
		...
		*/
		...
		//temp_writepara(ftemp,para1BS,ttmodel,countacc,i,1);// ; BS
		#pragma omp critical (updateflag)
		{
		  ...
		  // ; BS
		  // write BS or point model? maybe just point model
		  }
		}//critical (updateflag)
