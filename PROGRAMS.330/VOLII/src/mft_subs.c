...
extern float *period;
extern int do_auto_feed, num_period;
...
void do_showperscl(int i)
{
	/* This puts in the individual periods - this is useful for the
	   ...
	   than for velocity since sacmft96 worries about the USER1 and USER2 fields
	*/
	int j;
...
void do_showauto()
{
	...
	mgwrtxt(4.4,7.7,"  Autofeed  ",1,2);
	if(do_auto_feed == ON)
		mgwrtxt(4.4,7.4,"Autofeed On ",1,1);
	else
		mgwrtxt(4.4,7.4,"Autofeed Off",1,1);
}
...
*/
		...
		if(per >= 0.0){
			do_scale(per,vel,&xval,&yval,1);
			/*
			do_scale(dsp[i].instper,vel,&fxval,&yval,1);
			*/
