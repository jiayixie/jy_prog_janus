...
vector<paradef> paralst,paralstBS;
...
sprintf(dirlay,argv[2]);
  ...
  iitercri1=10000;//100000;//12000 (mod1, 1cstlay)
  iitercri2=15000;
  ijumpcri1=10;
  ijumpcri2=5;
  depcri1=20.0;
...
*/
    ...
    idlst.clear();
    printf("begin para_avg\n");//--check--
    if((para_avg_VOnly(paralst,parabest,tpara,parastd,LoveRAparastd,LoveAZparastd,idlst))==0){cout<<"#### in para_avg,incorrect paralst.size()\n";exit(0);}
    ttmodel=modelBS;
    paraBS=tpara;
    para2mod(paraBS,ttmodel,modelBS);
    updatemodel(modelBS,flagupdaterho);
    //---check---
    printf("after 1st do_inv===\n");
    for(i=0;i<paraBS.npara;i++){
	printf("  paraBS[%d]=%g  pflag=%d ng=%d\n",i,paraBS.parameter[i],(int)paraBS.para0[i][6],(int)paraBS.para0[i][4]);
    }//
