...
int main (int argc, char *argv[])
{
 ...
 static  double prpvper[300],prpvvel[300]; /* phase vel prediction files  */
  ...
  char  *p,name[160],buf[200],phvelname[160],root[160],amp_name[160];
  FILE  *in, *inv, *fas,*fwrong;
  int   i,pflag;
...
/* open a file to write the disp that failed aftan due to lack of input phasevel*/
 
  /*if((fwrong=fopen(argv[2],"w"))==NULL){
	printf("cannot open ftan_wrong.txt to write!");
	exit(-1);
  }
  */
...
/* open and read contents of parameter file  */
  ...
  while((n = fscanf(in,"%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %s %s",
            &piover4,&vmin,&vmax,&tmin,&tmax,&tresh,&ffact,&taperl,&snr,&fmatch,&pflag,
