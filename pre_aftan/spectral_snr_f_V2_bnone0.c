...
//  fprintf(stderr,"distance read from head file: %g\n",dist);
  ...
  if( maxT> ( e - 1000) )
    maxT= (e - 1000);
  window=maxT-minT;
//  fprintf(stderr,"HHHH minT=%g maxT=%g e=%g \n",minT,maxT,e);
/*
  minT=b;
  maxT=e;
  window=maxT-minT; 
...
//      for(i=(int)(maxT+500);i<e;i++)
      for(i=(int)(maxT+500-b);i<e-b;i++)
	{
//           fprintf(stderr,"%g %g %g %d %g\n",maxT + 500, e, sig0[i],i,b);
//	   abort();
	   sum = sum+sig0[i]*sig0[i];
//	   printf("SUM=%f i=%d sig0[i]=%f\n",sum,i,sig0[i]);
	}

      noisemax = sqrt(sum/(e-(int)(maxT+500)));
      snr[k]=signalmax/noisemax;      

//    fprintf(stderr,"signalmax=%g noisemax=%g sum=%g e-(maxT+500)=%g per=%g\n",signalmax,noisemax,sum,e-(int)(maxT+500),per[k]);
    }
