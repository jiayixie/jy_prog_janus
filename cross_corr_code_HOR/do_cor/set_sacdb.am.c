	...
	/*------------------------------------------------------------------------*/
{
	...
	sprintf(filename, "%s/ft_%s.%s.%s.SAC.am",sdb->ev[ne].name,sdb->st[ns].ntnm,sdb->st[ns].name, ch);

	if ( access(filename,F_OK) != 0) {printf("## Cannot access file %s\n",filename);return;}

	printf("------mk one rec--filename %s iev %d ista %d\n",filename, ne,ns);
...
//			sd.nzyear,   sd.nzjday,   sd.nzhour,    sd.nzmin,   sd.nzsec,   sd.nzmsec);
	...
	sprintf(sdb->rec[ne][ns].fname,"%s/ft_%s.%s.%s.SAC.am", sdb->ev[ne].name,sdb->st[ns].ntnm, sdb->st[ns].name, ch);
	sprintf(sdb->rec[ne][ns].ft_fname,"%s/ft_%s.%s.%s.SAC", sdb->ev[ne].name,sdb->st[ns].ntnm, sdb->st[ns].name, ch);
...
/*	int i;
	...
	printf(" lon %f lat %f\n",  st->lon, st->lat );
*/
   sscanf(buff,"%s %s %f %f",st->ntnm,st->name,&(st->lon),&(st->lat));
  fprintf(stderr,"--- fill sta: %s %s %f %f\n",st->ntnm,st->name,st->lon,st->lat);
...
/*	sscanf(&(buff[0]),"%d", &(ev->yy) );
	sscanf(&(buff[5]),"%d", &(ev->mm) );
	sscanf(&(buff[8]),"%d", &(ev->dd) );
...
*/	//ev->ms = 10.*ev->ms;
	...
	ev->ms = 10.*ev->ms;
        ...
        sscanf(buff,"%d %d %d",&(ev->yy),&(ev->mm),&(ev->dd));
	fprintf(stderr,"--- fill evt: %d %d %d ------\n",ev->yy,    ev->mm,   ev->dd);
...
//			ev->yy,    ev->mm,   ev->dd,    ev->h,    ev->m,    ev->s,     ev->ms);
	...
	sprintf(ev->name,"%04d_%02d_%02d_%d_%d_%d",ev->yy, ev->mm, ev->dd, ev->h, ev->m, ev->s );
	printf(" ev->t0 %f\n",ev->t0);
}
	...
	/*------------------------------------------------------------------------*/
{
	...
	if( argc != 4){
	printf("Usage: XXX 1)station.lst 2)event.dat 3)channel\n");
		return (0);
	}
	char channel[10];
	sscanf(argv[1],"%s",&station_lst[0]);
	sscanf(argv[2],"%s",&event_dat[0]);
	sscanf(argv[3],"%s",channel);
