...
//num_threads(1)
  ...
  for(int i=0;i<para.npara;i++){
	...
	*/
	...
	else if( model.groups[ng].flagcpttype==1 or model.groups[ng].flag==1 ){// cpt with Vkernel, or layered model;
	...
	{
  	printf("@@@ check, compute_Vkernel, end1 the %d the para\n",i);
	trkp2[i]=trkp1;
	trkg2[i]=trkg1;
	tlkp2[i]=tlkp1;
	tlkg2[i]=tlkg1;
...
// this is for iso or TI model, due to the computation capability of MINEOS 
  ...
  for(i=0;i<para.npara;i++){
	...
	if(LVflag==1){trkp1=Rp0;trkg1=Rg0;tlkp1=Lp0;tlkg1=Lg0;}// only need Vkernel,so Lkernel-->0
		...
		nv=(int)para.para0[i][5];
		ppflag=(int)para.para0[i][6];
		if(model.groups[ng].flag==1){//layered
			if(ppflag==9){//h
				trkp1=Vkernel[0][i];
				trkg1=Vkernel[1][i];
				tlkp1=Vkernel[2][i];
...
int write_kernel(vector<vector<vector<double> > > Vkernel,modeldef model0,paradef para1,char *kernelnmR, char *kernelnmL, int Rsurflag, int Lsurflag){
	...
	//################ ATTENTION, if you have lots of para, need to set tmpstr really big!
	char tmpstr[700];// this part need modification, the length should be defined based on # of para
	FILE *fkernel;
	int i,j;
	...
	if(Rsurflag>0){
		...
		*/
			...
			//fprintf(fkernel,"\n");
        		...
        		for(j=0;j<para1.npara;j++){sprintf(tmpstr,"%s %g",tmpstr,Vkernel[0][j][i]);}
			//printf("%s\n",tmpstr);
    	...
    	if(Lsurflag>0){
			...
			//printf("test--- write Lkernel %d\n",i);
        		for(j=0;j<para1.npara;j++){sprintf(tmpstr,"%s %g",tmpstr,Vkernel[2][j][i]);}
			//printf("%s\n",tmpstr);
...
// this code should probably use the para instead of group to fill VVparameter; if using group, then the need to do para2mod everytime before using this code, in order to guarantee that the model is updated;
  ...
  if (group.flag!=1){// not layered model
...
int Vpara2Lovepara(paradef &para,modeldef &model,int flagupdaterho){
  ...
  for(i=0;i<model.ngroup;i++){
	//---get the info for groups that need Lpara and is layered ---  
  	if((model.groups[i].flagcpttype-2)*(model.groups[i].flagcpttype-4)==0 and model.groups[i].flag==1){// group need Love para to do forward cpt, and group is layered
  ...
  //--get RA/AZ parameters for layered groups: for every group(m.ngroup) and layer(g.np) 
  ...
  for(i=0;i<para.npara;i++){
	...
	if(ppflag==9){//thickness
		...
		id=laygpid[ng];
		if(id>=0){// this is layered group and has Lpara
