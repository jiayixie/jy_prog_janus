...
//(NOT ANY MORE, can detect NphiC automatically now. b.c. I read in the para0 info) HEY, the code cannot detect the NphiC automatically, need to set it manually! remember to check if it is the right value before using!! (NphiC -- the NphiC's para, starts from 0, is phi in the crust)
...
{//for each input model, write out the vsv~phi~vpvs value from depmin(1st para) to depmax(2ed para) at depdx(3rd) interval.
  ...
  /*
  for(i=0;i<size;i++){
	fprintf(out,"%8g %8g %8g %8g %8g %8g %8g  %8g  ",vsv[i],vsh[i],vpv[i],vph[i],eta[i],theta[i],phi[i],hlst[i]);//---test---
  }*/
...
///*
  for (h=depmin;h<=depmax;h=h+depstep){
	...
	if (hlst[i]>h){
		//printf("h=%g hlst[i]=%g\n",h,hlst[i]);//--test--
		trho=(rho[i]-rho[i-1])/(hlst[i]-hlst[i-1])*(h-hlst[i-1])+rho[i-1];
...
int i,p6;
	...
	sprintf(kernelnmRHV,"%s/VkernelRHVp1ani_%s.txt",dirlay,name);
	...
	//sprintf(kernelnmR,"%s/LkernelRp1ani_%s_%.1f_%.1f.txt",dirlay,nodeid,lon,lat);
    	...
    	if(flagreadLkernel==1){
          ...
          write_kernel(Lkernel,model0,para1,kernelnmR,kernelnmL,kernelnmRHV,Rsurflag,Lsurflag);//added Apr 17, 2015
    	}//else
...
vector<vector<vector<double> > > Vkernel,Lkernel;
  ...
  */
  ...
  //---test---
/*
  sprintf(modnm1,"%s/Animod_%s_ref",dirlay,name);
  sprintf(fRdispnm,"%s/Rdisp_%s_ref",dirlay,name);
  sprintf(fLdispnm,"%s/Ldisp_%s_ref",dirlay,name);
  sprintf(fAZRdispnm,"%s/AZRdisp_%s_ref",dirlay,name);
  sprintf(fAZLdispnm,"%s/AZLdisp_%s_ref",dirlay,name);
  write_ASC(modelref,pararef,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,AziampRsurflag, AziampLsurflag,AziphiRsurflag, AziphiLsurflag);
*/	  
  return paraP;
...
// this version, we will calculate the uncertainty of anisotropy------	
  ...
  char Rphindir[100],Rgpindir[100],Lphindir[100],Lgpindir[100],modnm[100],PREMnm[100],dirlay[50],kernelnmR[200],kernelnmL[200];
  ...
  //---end of reading information
    ...
    if(RAflag==1){// recompute misfit based on the para (intrinsic); and write them out
      	...
      	for(j=0;j<N;j++){fprintf(fmis,"%g %g\n",newmisfitlst[j],paraall[j].misfit);}
      	fclose(fmis);
    }
    else{//RAflag=2; only have effective para, cannot compute misfit, so read from outside
	//--read the misfit file written in the RAflag=1 process
	sprintf(str,"%s/newmisfit_%.1f_%.1f.txt",outdir,lon,lat);
    ...
    for(j=0;j<N;j++){//don;t understand why cant I use &paraall to pass the parameters, so I have to pass the misfit through the misfitlst 
