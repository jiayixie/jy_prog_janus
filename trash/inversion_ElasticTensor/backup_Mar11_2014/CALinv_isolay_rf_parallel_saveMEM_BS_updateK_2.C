...
// this version, will also update the kernel at every jump; the code is still under construction
//-------
#include <sys/time.h>
...
int do_inv_BS(const int num_thread,const int id,const double misfitcri, vector<paradef> &paralst,const paradef refparaBS,const modeldef refmodelBS, paradef refpara, modeldef refmodel,const vector<int> Rvmono,const vector<int> Lvmono,const vector<int> Rvgrad,const vector<int> Lvgrad,const vector<vector<double> > PREM, vector<vector<vector<double> > > Vkernel, vector<vector<vector<double> > > Lkernel,const int k1,const int k2,const time_t start,const int isoflag,const int Rsurflag, const int Lsurflag,const int AziampRflag,const int AziampLflag,const int AziphiRflag,const int AziphiLflag,const int Nprem,const int Rmonoc,const int Lmonoc,const int PosAni, const vector<int> Vposani, const int iitercri,const int ijumpcri,const char *fbinnm,const float inpamp, const float inpphi, const int flagupdaterho){
  int tflag,iiter,iaccp,ibad,lastiaccp;
  ...
  //#pragma omp parallel default(none) shared(outbin,outbinRA,paralst,iloop,iiter,iaccp) private(tpara,para2,para1,ttmodel,tmodel,oldLL,oldRL,oldL,countitt,countacc,ibad,tflag,newL,newRL,newLL,newmisfit,now,prob,oldmisfit,prandom) 
  {
  ...
  for(int i=0;i<Njump;i++){
	...
	*/
	...
	for(countitt=0;countitt<iitercri;countitt++){
		...
		if (iaccp%500==0 and iaccp>0 and iaccp!=lastiaccp){// update the kernel after 1000 step walk in the model space, what if this model is a strange model??
			printf("re-compute Vkernel & Lkernel!!\n");
			lastiaccp=iaccp;
...
*/
			...
			compute_Vkernel(RApara,RAmodel,Vkernel,PREM,Nprem,Rsurflag,Lsurflag,flagupdaterho);
			...
			FILE *tmpfmod;
			char str2[200];
			sprintf(str,"refmod_iacc%d_iit%d.txt",iaccp,iiter);
			tmpfmod=fopen(str,"w");
			...
			fclose(tmpfmod);
			//----------*/
			sprintf(str,"VkernelR_iacc%d_iit%d.txt",iaccp,iiter);
			sprintf(str2,"VkernelL_iacc%d_iit%d.txt",iaccp,iiter);
			write_kernel(Vkernel,RAmodel,RApara,str,str2,Rsurflag,Lsurflag);
			sprintf(str,"LkernelR_iacc%d_iit%d.txt",iaccp,iiter);
			sprintf(str2,"LkernelL_iacc%d_iit%d.txt",iaccp,iiter);
