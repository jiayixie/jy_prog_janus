...
sprintf(tmpstr,"if [ ! -d %s/binmod ]; then mkdir %s/binmod; fi",dirlay,dirlay);
  ...
  while(1){
    ...
    Bsp2Point(model0,para1,modelP,paraP,flagupdaterho); // para1BS->para1; there is mod2para(Pmodel,Ppara) inside the Bsp2Point function
  ...
  write_ASC(model0,paraP,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,AziampRsurflag, AziampLsurflag,AziphiRsurflag, AziphiLsurflag);

//--------------finish computing the dispersion curves correlated with the input model ---
//exit(0);
...
//  char name[50];
  ...
  write_ASC(model1M,paraP,modnm1,fRdispnm,fLdispnm,fAZRdispnm,fAZLdispnm,Rsurflag,Lsurflag,AziampRsurflag, AziampLsurflag,AziphiRsurflag, AziphiLsurflag);
    ...
    //------
    model1K=model1;
    get_misfitKernel(model1K,para1,modelref,pararef,Vkernel,Lkernel,Rsurflag,Lsurflag,AziampRsurflag,AziphiRsurflag,AziampLsurflag,AziphiLsurflag,inpamp,inpphi,flagupdaterho);//contains Vpara2Lovepara and then compute_dispKernel, compute_misfitDISP
