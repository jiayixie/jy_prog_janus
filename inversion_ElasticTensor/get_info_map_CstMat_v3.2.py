...
#####
fpoint = "%s/point_info/point_info_closestnm_HsedGE5_1deg.txt"%(indir)
...
paraidlst=[7,8,77,78] #theta,phi in the crust, and theta, phi in the mantle
...
def get_aniC_aniM_fromV(fvsv,fvsh):
	...
	depmin=Hsedvsv
		...
		vs=sqrt((2*vsv**2+vsh**2)/3.)
                if(dep<depmin):
                        ani= (vsh-vsv)*200/(vsv+vsv)
			B=vsv+vsv
                        #ani= (vsh-vsv)*200/(vsv+vsh)
			#B=(vsv+vsh);
			#ani=(vsh-vsv)*100/vs;
                        A=(vsh-vsv);
                        dA=sqrt(uncvsh**2+uncvsv**2);
			#dB=dA
			dB=sqrt(uncvsv**2+uncvsv**2)
                        #unc=fabs(200/B**2*(B*dA-A*dB))
			unc=200/B**2*sqrt((B*dA)**2+(A*dB)**2)
                        #print dep,ani,unc
			...
			if (flag==0):
				#print "get ani mantle: dep=%f moho=%f"%(dep,Hmohovsv)
				#sys.exit()
                        	aniavg=sum/H;#crustal avg
                        	uncavg=sumunc/H;#crustal avg
				flag=1
				animat1=ani
				uncmat1=unc
				dep1=dep
			...
			Hmat+=(dep-dep1)
			animat1=animat;uncmat1=uncmat;dep1=dep;
...
def get_aniC_aniM(fani):
	...
	flag=0
        for line in open(fani):
				...
				if (flag==0):
                                	print "get ani mantle: dep=%f moho=%f"%(dep,Hmoho)
					sys.exit()
                                  	aniavg=sum/H; #crustal avg
                                  	uncavg=sumunc/H; #crustal avg
				  	flag=1
