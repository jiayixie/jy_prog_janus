...
int main(int na, char *arg[])
{
  if(na!=5)
    {
      cout<<"usage:correct_tr_t_curvature 1]event_name 2]period 3]least_sta_# 4]amp_curvature"<<endl;
      return 0;
    }
...
//  npts_y=61;
 ...
 y1=-999;
  ...
  sprintf(buff1,"%s.am.txt_v1.HD",arg[1]);
...
*/    
  ...
  marker_nn=0;
  int marker;
  sprintf(buff1,"%s.am.txt_v1",arg[1]);
...
*/
  for(;;)
      ...
      if(fscanf(fin3,"%lf %lf %lf %lf %lf",&lon2,&lat2,&temp2,&temp3,&temp4)==EOF) 
	{
      ...
      ist_old++;
      if(temp2<2*period)  // 1) KEEP stations whose travel time outside 2 period
	{
	  arr[ist][0]=lon2;
	  arr[ist][1]=lat2;
	  arr[ist][2]=temp2;
...
/*
      ...
      if(temp>atof(arg[4])||temp<-(atof(arg[4]))) // 2) IGNORE stations that is inside 2 period region, THROW those have travel-time curvature bigger than cur_cri
	{
 ...
 // fclose(fcur_biga);
  ...
  if(ist*1.0/ist_old<0.4)
	...
	cout<<"too less station left!!"<<endl;
      ...
      {
	sprintf(buff1,"%s.am.txt_v2",arg[1]);
	fout=fopen(buff1,"w");
	for(i=0;i<ist;i++)
	  {
	    fprintf(fout,"%lf %lf %lf %lf\n",arr[i][0],arr[i][1],arr[i][2],arr[i][4]); 
	  }
	fclose(fout);
      }
  return 0;
