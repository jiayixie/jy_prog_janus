...
int do_amp_to_laplace( char *event_name,double period, double x0, double y0, int npts_x, int npts_y,  char *indir,  char *outdir,  char *codedir)
{
/*  if(na!=7)
    {
      cout<<"usage:amp_gra_laplace_HD event.lst period x0 y0 npts_x npts_y"<<endl;
      return 0;
    }
*/
  ...
  double dx,dy,x1,y1,temp,temp1,temp2,lat_temp, temp3, temp4,temp_amp;
  ...
  //for(;;)
	...
	//break;
      sprintf(buff1,"%s/%s_am_laplace.txt.HD",outdir,event_name);
      if(access(buff1, F_OK) == 0)
	{
	  fprintf(stderr,"%s exist!! skip\n",buff1);
	  return -1;
	}
      sprintf(buff1,"%s/%s_v2_am.HD",indir,event_name);
      if((fin=fopen(buff1,"r"))==NULL)
       {
	 cout<<buff1<<" not exist!!"<<endl;
	 return -1;
       }
      sprintf(buff1,"%s/%s_am_gradx.txt_v2",outdir,event_name);
      if((fgradx=fopen(buff1,"w"))==NULL)
       {
	 cout<<buff1<<" cannot be written!!"<<endl;
	 return -1;
       }
      sprintf(buff1,"%s/%s_am_grady.txt_v2",outdir,event_name);
      ...
      for(i=0;i<npts_x;i++)
	{
      ...
      sprintf(buff1,"%s/%s_am",indir,event_name);
      ...
      for(;;)
	{
	  ...
	  temp2=(amp[i][j+1]-amp[i][j-1])/2.0/dy_km[j];
      ...
      fclose(fgradx);
      fclose(fgrady);
      sprintf(buff1,"%s/C_plot_travel %s/%s_am_gradx.txt_v2 %g %g %g %g",codedir,outdir,event_name,x0,x1,y0,y1);
      system(buff1);
      sprintf(buff1,"%s/C_plot_travel %s/%s_am_grady.txt_v2 %g %g %g %g",codedir,outdir,event_name,x0,x1,y0,y1);
      system(buff1);
      sprintf(buff1,"%s/%s_am_gradx.txt_v2.HD",indir,event_name);
      if((fgradx=fopen(buff1,"r"))==NULL)
	{
	  cout<<buff1<<" not exist!!"<<endl;
	  return -1;
	}
      sprintf(buff1,"%s/%s_am_grady.txt_v2.HD",indir,event_name);
      ...
      for(j=0;j<npts_y;j++)
	{
	  for(i=0;i<npts_x;i++)
	    {
              if (i==0 || i==npts_x || j==0 || j == npts_y)  {
                fprintf(fout,"%lf %lf -999\n",x0+i*dx,y0+j*dy);
                continue;
                }
	      if(amp[i+1][j]==999999||amp[i-1][j]==999999||amp[i][j+1]==999999||amp[i][j-1]==999999)
		{	 
	      ...
	      temp2=(amp_grady[i][j+1]-amp_grady[i][j-1])/2.0/dy_km[j];
