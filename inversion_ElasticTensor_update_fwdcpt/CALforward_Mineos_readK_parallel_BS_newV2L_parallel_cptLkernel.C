...
int computeLovekernel(vector<double> Vkernel1,double c, vector<double> &Lkernel1)
int computeLovekernel2(vector<double> Vkernel1,vector<double> Vkernel2,double c1,double c2,vector<double> &Lkernel1)
...
int L2Vpara(paradef &para, int ipara)
...
int compute_Lkernel(paradef para,modeldef model,vector<vector<vector<double> > > &kernel,const vector<vector<double> > PREM,const int Nprem, const int Rflag,const int Lflag,const int flagupdaterho, const int ijump)

*/
// this version, use the new formula(derived from full differentiation) to compute the Lkernel from Vkernel. Mar 4, 2014; modified  compute_RAdisp  Vkernel2Lkernel computeLovekernel
// this version, modify the parallel situation in compute_Vkernel, set the thread used, and in order to allow multi compute_Vkernel to work at the same time, change the way i name the output files
// this version, add the compute_Lkernel function, that computes Lkernel directly without using Vkernel
...
//num_threads(2)
  ...
  for(int i=0;i<para.npara;i++){
	...
	*/
	...
	if(LVflag==1 and para.space1[i][2]*(para.space1[i][2]-0.001)<0.){// if it won't be used to cpt LoveKernel and para don't need to be perturbed or scaled; then no need to cpt its partial deriv
	...
	else if( model.groups[ng].flagcpttype==3 ){// cpt with V+AZ kernel  (no need Love kernel)
		if(ppflag==10 or ppflag==11){// by default, ppflag is increasing, so ppflag==1 for this group has kernel computed already , BUT ATTENTION, THIS PART NEED CHANGE IN PARALLEL CASE!
			...
			*/
			...
			printf("@@@ check, compute_Vkernel, dealing with the %d/%dth para, DOOOOO Vkernel cpt=====\n",i,para.npara-1);
			compute_Vkernel_single_para(para,i,model,PREM,Nprem, Rflag,Lflag,flagupdaterho,trkp1,trkg1,tlkp1,tlkg1,i+ijump*para.npara);
       ...
       //else if( model.groups[ng].flagcpttype==1 or model.groups[ng].flag==1 or model.groups[ng].flag==6 ){// cpt with Vkernel, or layered model, or point model; BS
       	else if (model.groups[ng].flagcpttype==4){// all cpt with Lkernel
			trkp1=Rp0;trkg1=Rg0;tlkp1=Lp0;tlkg1=Lg0;
                printf("@@@ check, compute_Vkernel, dealing with the %d/%dth para, NO Vkernel cpt=====\n",i,para.npara-1);		
       	}//if 4
       	else if( model.groups[ng].flagcpttype==1 ){// cpt with Vkernel
	...
	{
  	printf("@@@ check, compute_Vkernel, end of the %d/%d th para\n",i,para.npara-1);
	trkp2[i]=trkp1;
	trkg2[i]=trkg1;
	tlkp2[i]=tlkp1;
	tlkg2[i]=tlkg1;
...
//---------------------------------------
int computeLovekernel(vector<double> Vkernel1,double c,vector<double> &Lkernel1){//modified Mar 4, 2014; for the new V2Lkernel formula 
...
//---------------------------------------
int computeLovekernel2(vector<double> Vkernel1,vector<double> Vkernel2,double c1, double c2,vector<double> &Lkernel1){//modified Mar 4, 2014; for the new V2Lkernel formula 
  //compute the Lkernel (dc/dX) from the Vkernel (dc/dV)
  int i;
  Lkernel1.clear();
  for(i=0;i<Vkernel1.size();i++){
	Lkernel1.push_back(Vkernel1[i]*c1+Vkernel2[i]*c2);
  }//for i 
  return 1;
}//convertVkernel2Lovekernel
//---------------------------------------
...
// this is for iso or TI model, due to the computation capability of MINEOS 
  ...
  vector<double> trkp1,trkg1,tlkp1,tlkg1;
  vector<double> Rp0(model.data.Rdisp.npper,0.),Rg0(model.data.Rdisp.ngper,0.),Lp0(model.data.Ldisp.npper,0.),Lg0(model.data.Ldisp.ngper,0.);
  double rho,A,L,tA,tL,Vv,c1,c2;

  Lkernel.clear();
  if(flagupdaterho==1){// need to change rho accordingly
	...
	if(LVflag==1){trkp1=Rp0;trkg1=Rg0;tlkp1=Lp0;tlkg1=Lg0;}// only need Vkernel,so Lkernel-->0
		...
		ppflag=(int)para.para0[i][6];
			...
			if(ppflag==9){//h
				...
				c1=1./(A-2*L);
				//printf("eta c=%g=1/(%g-2*%g)\n",c1,A,L);
				computeLovekernel(Vkernel[0][i],c1,trkp1);
				computeLovekernel(Vkernel[1][i],c1,trkg1);
				computeLovekernel(Vkernel[2][i],c1,tlkp1);
				computeLovekernel(Vkernel[3][i],c1,tlkg1);
				
			}
			else if (ppflag==1){ //vsv, L; dc/dL=1/(2*rho*vsv)*dc/dvsv+2*F/(A-2L)^2*dc/deta
				rho=model.groups[ng].rhovalue[nv];
				c1=1/(2*rho*model.groups[ng].vsvvalue[nv]);//1/(2*rho*vsv)
				A=rho*pow(model.groups[ng].vphvalue[nv],2.0);
				L=rho*pow(model.groups[ng].vsvvalue[nv],2.0);
				//printf("vsv A L from model=%g %g %g; vsv A L from_para=%g %g %g\n",model.groups[ng].vsvvalue[nv],A,L,para.parameter[i],para.LoveRAparameter[i+3],para.LoveRAparameter[i]);//--test
				//if(fabs(model.groups[ng].vsvvalue[nv]-para.parameter[i])>1e-5)exit(0);
				c2=2*model.groups[ng].etavalue[nv]/(A-2*L);//2*F/(A-2*L)^2=2*eta/(A-2L)
				computeLovekernel2(Vkernel[0][i],Vkernel[0][i+4],c1,c2,trkp1);
				computeLovekernel2(Vkernel[1][i],Vkernel[1][i+4],c1,c2,trkg1);
				computeLovekernel2(Vkernel[2][i],Vkernel[2][i+4],c1,c2,tlkp1);
				computeLovekernel2(Vkernel[3][i],Vkernel[3][i+4],c1,c2,tlkg1);
			}
			else if (ppflag==4){ //vph, A; dc/dA=1/(2*rho*vph)*dc/dvph-F/(A-2L)^2*dc/deta
				rho=model.groups[ng].rhovalue[nv];
				c1=1./(2*rho*model.groups[ng].vphvalue[nv]);
				A=rho*pow(model.groups[ng].vphvalue[nv],2.0);
                                L=rho*pow(model.groups[ng].vsvvalue[nv],2.0);
				c2=-1*model.groups[ng].etavalue[nv]/(A-2*L);//-F/(A-2L)^2=-eta/(A-2L)
                                computeLovekernel2(Vkernel[0][i],Vkernel[0][i+1],c1,c2,trkp1);
                                computeLovekernel2(Vkernel[1][i],Vkernel[1][i+1],c1,c2,trkg1);
                                computeLovekernel2(Vkernel[2][i],Vkernel[2][i+1],c1,c2,tlkp1);
                                computeLovekernel2(Vkernel[3][i],Vkernel[3][i+1],c1,c2,tlkg1);

			}
			else if ((ppflag-2)*(ppflag-3)==0){//vsh(N) or vpv(C)
				rho=model.groups[ng].rhovalue[nv];
				Vv=para.parameter[i];//value in the Vparameter (velocity)
				A=1./(2*rho*Vv);
				//Lv;//value in the Loveparameter (elastic coefficient)
				computeLovekernel(Vkernel[0][i],A,trkp1);
				computeLovekernel(Vkernel[1][i],A,trkg1);
				computeLovekernel(Vkernel[2][i],A,tlkp1);
				computeLovekernel(Vkernel[3][i],A,tlkg1);
				//---check---
...
{ //from the RA part of the Love para, get vsv~vph,eta & theta=pho=0;
  ...
  for(i=0;i<para.npara;i++){
	...
	if(model.groups[ng].flagcpttype==2 or model.groups[ng].flagcpttype==4){//this group needs to cpt RA disp using the Vparameters, so this Lpara2Vpara is needed, (for the cpttype==4 case, the computation of F_partial_deriv needs information of dL, dA, dEta, so need the effective Vpara!)
		ppflag=(int)para.para0[i][6];
		//printf("L2Vpara the %dth para, ppflag=%d ng=%d\n",i,ppflag,ng);//---check---
		if((ppflag-7)*(ppflag-6)==0){//ppflag=6 or 7, theta or phi
			...
			L=para.LoveRAparameter[i-4];//--check--
			A=para.LoveRAparameter[i-1];//--check--
			/*if(L<0 or A<0){
				printf("### Lovepara2Vpara, the L and A haven't been defined!! ppflag=%d, the %dth para,ng=%d\n",ppflag,i,ng);	
				exit(0);
			}*/
...
//---------------------------------------
/*
//---modify, Jan 17, 2014. get_index functoin moved to CALpara_isolay_BS.C
...
*/
//---------------------------------------
vector<vector<double> >  get_kernel_subset(vector<vector<vector<double> > > kernel, vector<int> index, int n){ // get the subset of kernel[n] ([ipara][iperiod]) based on idex (list of iperiod)
  vector<vector<double> > subkernel;
  vector<double> v1;
  int ipara,iT,i;
  int Nid=index.size();
  int Npara=kernel[n].size();
  for(ipara=0;ipara<Npara;ipara++){
	v1.clear();
	for(i=0;i<Nid;i++){
		iT=index[i];
		v1.push_back(kernel[n][ipara][iT]);
	}
	subkernel.push_back(v1);
  }
...
int compute_AZdisp(modeldef &model, paradef para, modeldef refmod, paradef refpara, vector<vector<vector<double> > > Vkernel, vector<vector<vector<double> > > Lkernel, int Razflag, int Lazflag){
  ...
  //(this is wrong! the angle in M&N's paper is right)since I think the angle in Montagner&Nataf(1986) paper is CCW from S, (not CW from N, as they stated in the paper), I need to modify the phi computed from Loveparameters, but no need to do modifications to the phi computed from AZcos, AZsin. 
  ...
  for(i=0;i<para.npara;i++){
	  if((int)para.para0[i][7]==1)
	  	printf("\trefpara%d=(%8.3f,%8.3f), paradiff=(%8.3f,%8.3f) Rflag=%3d Lflag=%3d\n",i,refpara.parameter[i],refpara.LoveAZparameter[i][1],Cosparadiff[i],Sinparadiff[i],Rflag[i],Lflag[i]);
	  else
	  	printf("\trefpara%d=(%8.3f,%8.3f), paradiff=(%8.3f,%8.3f) Rflag=%3d Lflag=%3d Lovepara\n",i,refpara.LoveAZparameter[i][0],refpara.LoveAZparameter[i][1],Cosparadiff[i],Sinparadiff[i],Rflag[i],Lflag[i]);
  }
  */
  ...
  //---modify, Jan 17, 2014. perlst of Rdisp and AziampRdisp/AziphiRdisp could be different, should take this into consideration.
  vector<double> period1;
  vector<int> index;
  vector<vector<double> >  kernelRPamp,kernelRPphi,kernelLPamp,kernelLPphi; // the kernels are only for phase speed

  period1=model.data.Rdisp.pper;
  index=get_index(period1,model.data.AziampRdisp.pper);
  kernelRPamp=get_kernel_subset(kernel,index,0);
  index=get_index(period1,model.data.AziphiRdisp.pper);
  kernelRPphi=get_kernel_subset(kernel,index,0);

  period1=model.data.Ldisp.pper;
  index=get_index(period1,model.data.AziampLdisp.pper);
  kernelLPamp=get_kernel_subset(kernel,index,2);
  index=get_index(period1,model.data.AziphiLdisp.pper);
  kernelLPphi=get_kernel_subset(kernel,index,2);

   //test----
   char strsin[300],strcos[300];
  //---modify Jan 17, 2014, the following part could be much brief, but i haven't done that yet, do it later--
  //---compute R AZdisp ---
...
//-----------------------------------------------------
int L2Vpara(paradef &para, modeldef model,int ipara){//LoveRA para to Vpara, only apply changed to related parameters
  int i,inng,innv,inppflag,idvsv,idrho;
  double rho;
  //ipara indicates the para that have been perturbed, so I just need to concentrate on the parameters that are related to this ipara.
  // right now, only consider the change in ACFLN and thickness
  inng=(int)para.para0[ipara][4];
  innv=(int)para.para0[ipara][5];
  inppflag=(int)para.para0[ipara][6];


  //get vsv~rho based on the inppflag (which tells the type of this para(1-5, LNCAF, or Vsv Vsh Vpv Vph eta))
  ...
  if(inppflag==9){//thickness
	para.parameter[ipara]=para.LoveRAparameter[ipara];
  }
  else if((inppflag-2)*(inppflag-3)==0){// N or C para, they only affect the corresponding vel (vsh or vpv )
	para.parameter[ipara]=sqrt(para.LoveRAparameter[ipara]/rho);
  }
  else if ((inppflag-5)==0){// F
	para.parameter[ipara]=para.LoveRAparameter[ipara]/(para.LoveRAparameter[idvsv+3]-2*para.LoveRAparameter[idvsv]);//eta=F/(A-2L)
	
  }
  else if ((inppflag-1)*(inppflag-4)==0){// L or A para; they affect both corresponding vel and eta
	para.parameter[ipara]=sqrt(para.LoveRAparameter[ipara]/rho);
	para.parameter[idvsv+4]=para.LoveRAparameter[idvsv+4]/(para.LoveRAparameter[idvsv+3]-2*para.LoveRAparameter[idvsv]);
  }
  else{// rho, theta, phi, AZcos, AZsin -- how to consider the updaterho situation here?
	// some parameters has LVflag==1 (use Vpara&Vkernel to do all forward computation) or has ppflag>9, then the Love parameters are all 0
	printf("para%d  ng=%d nv=%d ppflag=%d, don;t need to do L2Vpara\n",ipara,inng,innv,inppflag);
  }//else
...
// perturb para.LoveRAparameter --> L2Vpara --> get delta_disp --> get partial derivatives
  ...
  return 1;
}//compute_Lkernel_single_para
//-----------------------------------------------------
int compute_Lkernel(paradef para,modeldef model,vector<vector<vector<double> > > &kernel,const vector<vector<double> > PREM,const int Nprem, const int Rflag,const int Lflag,const int flagupdaterho, const int ijump){
// this computes the Lkernel directly from Minoes without the need to compute Vkernel 
// only do this to point(from BS model) or layered model (model.group[].flag=1 or 6)
  ...
  #pragma omp for
  for (int i=0;i<para.npara;i++){
	//trkp1.clear();trkg1.clear();tlkp1.clear();tlkg1.clear(); // will do this in the compute_Lkernel_single_para, so rm it here
	ng=(int)para.para0[i][4];
        nv=(int)para.para0[i][5];
        ppflag=(int)para.para0[i][6];
	LVflag=(int)para.para0[i][7];
	...
	if(LVflag==1 or model.groups[ng].flagcpttype==3 or model.groups[ng].flagcpttype==1){trkp1=Rp0;trkg1=Rg0;tlkp1=Lp0;tlkg1=Lg0;}// only need Vkernel,so Lkernel-->0; or cpt with V+AZ kernel; or cpt with Vkernel
		...
		printf("####, compute_Lkernel, computation for this situation is still under construction! Sorry!\n");
		exit(0);
	}
	else{
                printf("####, compute_Lkernel, this is an unconsidered situation, consider it NOW!\nOR add limits to the checkParaModel function to prevent this situation from happening\n");
                exit(0);
        }//else
	#pragma critical (printout)
	{
  ...
  //---check---
  if(trkp2.size()!=para.npara){
        printf("@@@ check compute_Lkernel, the size of kernel(%d) isn't right(%d)!! \n",trkp2.size(),para.npara);
        exit(0);
  }//if 
  kernel.push_back(trkp2);
  kernel.push_back(trkg2);
