...
c medium. Fixed from Farra et al., 1991
        ...
        do ilay=1,nlay
          ...
          write(*,*) 'top of ilay loop in buildmodel: ilay=',ilay
...
creadtensor c Build anisotropic coefficients for a hexagonally-symmetric medium,
creadtensor c after Farra et al, 1991
creadtensor             d_a=alpha(ilay)*pct_a(ilay)/100.
creadtensor             d_b=beta(ilay)*pct_b(ilay)/100.
creadtensor             AA=rho(ilay)*(alpha(ilay) - d_a/2.)**2.
creadtensor             CC=rho(ilay)*(alpha(ilay) + d_a/2.)**2.
creadtensor             LL=rho(ilay)*(beta(ilay) + d_b/2.)**2.
creadtensor             NN=rho(ilay)*(beta(ilay) - d_b/2.)**2.
creadtensor             FF=eta*(AA-2.*LL)
creadtensor c            write (*,*) 'Anis. params are:',AA,CC,FF,LL,NN
creadtensor c Get tensor with unrotated axes
creadtensor             call tritensr(a_temp,AA,CC,FF,LL,NN,rho(ilay))
            write(*,*) 'filltensor calls: ilay=',ilay,isoflag(ilay)
            if(ilay .eq. 2) then
                write(*,*) 'ilay=',ilay,' reading tensortop'
                call filltensor(a_temp,tensortop,handedness)
            endif
            if(ilay .eq. 3) then
                write(*,*) 'ilay=',ilay,' reading tensormid'
                call filltensor(a_temp,tensormid,handedness)
            endif
            if(ilay .eq. 4) then
               write(*,*) 'ilay=',ilay,' reading tensorbottom'
               call filltensor(a_temp,tensorbottom,handedness)
            endif
            if (ilay .gt. 4) then
                write(*,*) 'ilay=',ilay,' too many aniso tensors, stopping'
                write(*,*) 'only 3 anisotropic tensors for layers 2-4 allowed'
                stop
            endif
...
c            write (*,*) 'rot_axis:',rot_axis
            write(*,*) 'rotating, trend ',trend(ilay),' plunge ',plunge(ilay)
