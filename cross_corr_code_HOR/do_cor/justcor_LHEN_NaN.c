...
/*--------------------------------------------------------------------------*/
{
  ...
  char cp_am_ph[200];
  char buff[300];
  sprintf(buff,"if [ ! -d CORNAN ]; then mkdir CORNAN; fi");
  system(buff);

  for( jsta1 = 0; jsta1 < istNaN; jsta1++ ) 
    {  
      fprintf(stderr,"---jsta1 NaN%d: %s--%d%d\n",jsta1,sta_nameNaN[jsta1],iii,kkk);
      sprintf(buff,"if [ ! -d CORNAN/%s ]; then mkdir CORNAN/%s; fi",sta_nameNaN[jsta1],sta_nameNaN[jsta1]);
      system(buff);
      for( jsta2 = (jsta1+1); jsta2 < istALL; jsta2++ )
	{	
	  count=0;
	  if(iii==0)
	    return 0;
	    ...
	    {
	      sprintf(filename, "CORNAN/%s/COR_%s_%s.SAC_N",
		      sta_nameNaN[jsta1], sta_nameNaN[jsta1], sta_nameALL[jsta2]);
	    }
	  else if(iii==2)
	    {
	      sprintf(filename, "CORNAN/%s/COR_%s_%s.SAC_E",
		      sta_nameNaN[jsta1], sta_nameNaN[jsta1], sta_nameALL[jsta2]);
	    }
	  if(kkk==0)
	    return 0;
