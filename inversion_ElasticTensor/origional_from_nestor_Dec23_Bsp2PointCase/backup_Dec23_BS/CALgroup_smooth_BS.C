...
/*========CONTENT===========
this is for the smooth model instead of layered model (for MINEOS instead of Hermman's code, so it is not really layerize the model, but use points to represent the model)
Here,for the BSpline model: there are some 0-thickness layers 
and for the layered model: the nlay of the layered(pointed) model is doubled
...
int updategroupBsPoint(groupdef &group)
int updategroup4(groupdef &group)
int updategroup5(groupdef &group)
//==========================
*/
//class groupcal{
//public:
...
//----------------------------------------------------- 
        int updategroupBsPoint(groupdef &group) //calculate the Bspline function     ; BS
        {//this is the same as updategroupBsPoint except that nlayer is set to 10
          int nBs,order;
          double factor;
	  group.nlay=3;
          nBs=group.np;
...
//
	  ...
	  group.rhovalue1.clear();
          ...
          if(group.flagBs!=1){ //; BS
		if(group.flag==2)updategroupBs(group);
		else if(group.flag==3)updategroupBsPoint(group);
	  }
	  nnlay=group.nlay;
	  nBs=group.np;
	  for (i=0;i<nnlay-1;i++)
...
//----------------------------------------------------- 
	int updategroup5(groupdef &group) // for the point model; BS
	{
	  ...
	  group.vphvalue1=group.vphvalue;
	  group.etavalue1=group.etavalue;
	  group.thetavalue1=group.thetavalue;
...
//----------------------------------------------------- 
	  ...
	  else if((group.flag-2)*(group.flag-3)==0)updategroup2(group);//Bs
	  else if(group.flag==4)updategroup3(group);//grad
	  else if (group.flag==5)updategroup4(group);//water layer
	  else if (group.flag==6)updategroup5(group);//point; BS
