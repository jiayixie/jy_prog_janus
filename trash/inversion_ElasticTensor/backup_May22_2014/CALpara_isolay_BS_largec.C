...
//============================
// this version, when the sigma for eta<-5, then I set c=2 or 1 (c measures the non-ellipticity of the tensor). modified Feb 18, 2014
//
*/
//class paracal{
//public:
...
//-----------------------------------------------------	 
	int checkParaModel(paradef para, modeldef model){
	  ...
	  for(i=0;i<model.ngroup;i++){
		...
		if(para.space1[j][2]<-3. and para.space1[j][2]>=-5.){//non-linear scaling based on the 1st value in that group during the para2mod process
		  ...
		  if(p6==1){
			  ...
			  if(para.space1[i][2]<-1.)// non-linear scale; use the RAvp, eta, RAvs relation ; BS
			  {
			   ...
			   if(para.space1[i][2]<-1. )//non-linear scale; use the RAvp, eta, RAvs relation ; BS
			   {
				  ...
				  if( (gflag-2)*(gflag-3)!=0){//---check---
					if (para.space1[i][2]<-5.){//modified Feb 18, 2014
						// use large c value to compute eta (c measures the non-ellipticity and c=1 or 2) eta=(vph^2+vpv^2-4*vsv^2-8c/rho)/(2*(vph^2-2*vsv^2)); and rho = 0.541+0.3601*0.5*(vpv+vph)
						cc=2;//the c modified c value
						factor=0.541+0.3601*0.5*(outmodel.groups[ng].vpvvalue[nv]+outmodel.groups[ng].vphvalue[nv]);//rho
						factor=(pow(outmodel.groups[ng].vphvalue[nv],2)+pow(outmodel.groups[ng].vpvvalue[nv],2)-4*pow(outmodel.groups[ng].vsvvalue[nv],2)-8*cc/factor);//(vph^2+vpv^2-4*vsv^2-8c/rho)
						outmodel.groups[ng].etavalue[nv]=0.5*factor/(pow(outmodel.groups[ng].vphvalue[nv],2)-2*pow(outmodel.groups[ng].vsvvalue[nv],2));//eta
					}
					else if(para.space1[i][2]<-3.){
