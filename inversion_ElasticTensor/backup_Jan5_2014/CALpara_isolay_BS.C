...
//-----------------------------------------------------	 
	int checkParaModel(paradef para, modeldef model){
	  ...
	  for(i=0;i<model.ngroup;i++){
		...
		if(para.space1[j][2]<-3.){//non-linear scaling based on the 1st value in that group during the para2mod process
	  ...
	  for(i=0;i<para.npara;i++){
		...
		if(p0==0){ //value
		  ...
		  if(p6==1){
			  ...
			  // if sigma<-1; then 1) if non-Bspline crust, then use RAvp-RAvs relation to get vph; 2) if mantle or Bspline crust, then vph=vpv
			  if(para.space1[i][2]<-1.)// non-linear scale; use the RAvp, eta, RAvs relation ; BS
			  {
			      	if((gflag-2)*(gflag-3)!=0 and ng<2){//non-linear scaling for non-Bsp crust(sed+crystalline crust)    ; BS
			   ...
			   if(para.space1[i][2]<-1. )//non-linear scale; use the RAvp, eta, RAvs relation ; BS
			   {
				  if(ng<2 and (gflag-2)*(gflag-3)!=0){
					if(para.space1[i][2]<-3.){
