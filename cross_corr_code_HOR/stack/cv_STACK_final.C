...
#include <iostream>
#include "/home/weisen/PROGS_64/noise_coda/NOISE_DATABASE/mysac.h"
...
void sac_add ( SAC_HD *shd, float *fsig, char *filename, int flag, float kk ) {
  int ns,i;
  float tsig[1000000];
  SAC_HD shd1;
  ...
  if (flag == 1) {
    for (i=1;i<=3000;i++) {
	fsig[i+3000] = fsig[i+3000] + tsig[(ns-1)/2 + i] * kk;
	fsig[3000-i] = fsig[3000-i] + tsig[(ns-1)/2 - i] * kk;
        }
    fsig[3000] = fsig[3000] + tsig[(ns-1)/2] * kk;
    *shd = shd1;
    }
  else if (flag == 0 ) {
...
int main (int argn, char *argv[]) {
  ...
  char tname3[300],tname4[300],aray[NSTA][5];
...
//  if (kk<0) kk = 1.;
  ...
  for (i=0;;i++) {
    if (fscanf(ff1,"%s %g %g %s", &staname[i], &stalon[i], &stalat[i],&aray[i]) != 4) break;
//    cout<<staname[i]<<" "<<aray[i]<<endl;
    }
  nsta = i;
  fclose(ff1);  
...
//       cout<<"ndir "<<ndir<<endl;
	 ...
	 sprintf(tname3,"%s/%s.%s/COR_%s.%s_%s.%s.SAC\0",dirlist[k],aray[i],staname[i],aray[i],staname[i],aray[j],staname[j]);
	 sprintf(tname4,"%s/%s.%s/COR_%s.%s_%s.%s.SAC\0",dirlist[k],aray[j],staname[j],aray[j],staname[j],aray[i],staname[i]);
         kk = kflag[k];

//	 cout<<tname1<<" "<<tname2<<" "<<tname3<<" "<<tname4<<" "<<endl;
