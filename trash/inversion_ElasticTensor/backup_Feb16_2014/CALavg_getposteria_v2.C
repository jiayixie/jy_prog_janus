	  ...
	  for(th=h0;th<=h+dth;th=th+dth){//thickness
		...
		//	dep1=dep1+dep1lst[i]+modlst[i].laym0.thick[j];
			if(dep1>=tth ){//  the interpolation, there was a bug here (for smooth model, though it works for layered model), changed on Aug27, 2012. another bug modified on Oct 8, 2013
				//if(flag==1)printf("dep1=%7f tth=%7f\n",dep1,tth);//--test---
...
//--------------------------------------------------
       int model_avg2( const char *fvsvnm, const char *fvshnm,char *faninm, vector<modeldef> &modlstall, vector<int> &idlst)
	{
	  ...
	  /*---test
	  printf("Hsed=%7f(%5f) Hmoho=%7f(%5f) depmax=%7f\n",Hsed,Hsedstd,Hmoho,Hmohostd,depmax);
	  for(i=0;i<v1lst.size();i++){
		printf("h1=%7.3f v1=%7.3f\n",h1lst[i],v1lst[i][0]);
  	  }//
	  for(i=0;i<v2lst.size();i++){
		if(i<2 or i>v2lst.size()-3)
		printf("h2=%7.3f v2=%7.3f\n",h2lst[i],v2lst[i][0]);
  	  }//
	  for(i=0;i<3;i++){
		printf("h3=%7.3f v3=%7.3f\n",h3lst[i],v3lst[i][0]);
  	  }*/
	  ...
	  //write out the depth of sed, moho and the velocities(depth)
...
// this version, we will calculate the uncertainty of anisotropy------	
  ...
  char fbname1[300],fvsvnm1[300],fvshnm1[300],foutnm1[300],fparanm[300];
  char faninm[300];
  FILE *inpo, *out;
  int npoint,i,RAflag;
  float tvsv,tvsh,tani;
  vector<float> anilst;	  
  paradef paraavg,parabest;
  float lon,lat;  
  ...
  while(1){
    ...
    if((para_avg_V(paraall,parabest,paraavg,parastd,idlst))==0){cout<<"### in para_avg, incorrect paralst.size()\n";exit(0);}
    printf("size=%d\n",idlst.size());
    ...
    fclose(out);
    /*for(i=0;i<paraavg.npara;i++){
	printf("i=%d, paraavg=%7f, std=%7f\n",i,paraavg.parameter[i],parastd[i]);
    }*/
    //exit(0);
