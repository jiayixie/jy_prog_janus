...
int do_inv_BS(const int num_thread,const int id,const double misfitcri, vector<paradef> &paralst,const paradef refparaBS,const modeldef refmodelBS, const paradef refpara0,const modeldef refmodel0,const vector<int> Rvmono,const vector<int> Lvmono,const vector<int> Rvgrad,const vector<int> Lvgrad,const vector<vector<double> > PREM, const vector<vector<vector<double> > > Vkernel0,const vector<vector<vector<double> > > Lkernel0,const int k1,const int k2,const time_t start,const int isoflag,const int Rsurflag, const int Lsurflag,const int AziampRflag,const int AziampLflag,const int AziphiRflag,const int AziphiLflag,const int Nprem,const int Rmonoc,const int Lmonoc,const int PosAni, const vector<int> Vposani, const int iitercri,const int ijumpcri,const char *fbinnm,const float inpamp, const float inpphi, const int flagupdaterho){
  int tflag,iiter,iaccp,ibad,lastiaccp,iaccpprivate;
  ...
  omp_set_num_threads(int(num_thread/10)); // Use 4 threads for all consecutive parallel regions, divide it by 4 to leave some cpu space for the kernel computation
  //omp_set_num_threads(num_thread);
  }
  while(iloop<10 and iaccp<accpcri){
  //while(iloop<10 and iaccp<accpcri){
  ...
  #pragma omp parallel default(none) shared(outbin,outbinRA,paralst,iloop,iiter,iaccp) private(tpara,para2,para1,ttmodel,tmodel,oldLL,oldRL,oldL,countitt,countacc,ibad,tflag,newL,newRL,newLL,newmisfit,now,prob,oldmisfit,prandom,refmodel,Vkernel,Lkernel,lastiaccp,refpara,iaccpprivate) 
  {
  ...
  for(int i=0;i<Njump;i++){
	...
	*/
	...
	for(countitt=0;countitt<iitercri;countitt++){
	  	iiter++;
		if(iiter%10000==0){
			printf("iiter=%d iaccp=%d dtime=%.1f ijump=%d iloop=%d; iaccpprivate=%d  ",iiter,iaccp,(float)(time(0)-start),i,iloop,iaccpprivate);
			printf("inv time used wtime=%.2fs cputime=%.2fs\n",omp_get_wtime()-start_walltime,get_cpu_time()-start_cputime);
		}
		if (iaccpprivate%10==0 and iaccpprivate>0 and iaccpprivate!=lastiaccp){// update the kernel after X step walk in the model space, what if this model is a strange model??
		//#pragma omp critical (updateKernel)
		{
			printf("re-compute Vkernel & Lkernel!! iaccpprivate=%d\n",iaccpprivate);
			lastiaccp=iaccpprivate;
			...
			//ipara?? Rflag?? Lflag??
			//updatemodel()--> the model.laym0.X are for flat model? I need that for the eff. TI model
...
*/
			//ttmodel,para1
			//use RApara, RAmodel, Vkernel & Lkernel should be private variables
			//printf("RAmodel.flag=%d\n",RAmodel.flag);//--test--
			//printf("hey cpt_dispM\n");
			...
			/*--test---output the parameter change
			printf("RAparameter:\n");
			for(int m=0;m<RApara.npara;m++){
				printf("para%d, parameter: %5f->%5f LoveRA %5f->%5f; LoveAZ cos %5f->%5f sin %5f->%5f\n",m,refpara.parameter[m],RApara.parameter[m],refpara.LoveRAparameter[m],RApara.LoveRAparameter[m],refpara.LoveAZparameter[m][0],RApara.LoveAZparameter[m][0],refpara.LoveAZparameter[m][1],RApara.LoveAZparameter[m][1]);
			}
			------*/
			...
			FILE *tmpfmod;
			char str2[200];
			sprintf(str,"refmod_ijump%d_iloop%d_iacc%d_iit%d.txt",ijump,iloop,iaccpprivate,iiter);
			tmpfmod=fopen(str,"w");
			...
			fclose(tmpfmod);
			//----------*/
			sprintf(str,"VkernelR_ijump%d_iloop%d_iacc%d_iit%d.txt",ijump,iloop,iaccpprivate,iiter);
			sprintf(str2,"VkernelL_ijump%d_iloop%d_iacc%d_iit%d.txt",ijump,iloop,iaccpprivate,iiter);
			write_kernel(Vkernel,RAmodel,RApara,str,str2,Rsurflag,Lsurflag);
			sprintf(str,"LkernelR_ijump%d_iloop%d_iacc%d_iit%d.txt",ijump,iloop,iaccpprivate,iiter);
			sprintf(str2,"LkernelL_ijump%d_iloop%d_iacc%d_iit%d.txt",ijump,iloop,iaccpprivate,iiter);
			write_kernel(Lkernel,RAmodel,RApara,str,str2,Rsurflag,Lsurflag);
			
		}//critical updateKernel
		}//update kernel
		...
		//para2=para1;
  		//-----------------Criteria 1, goodmodel------------------------
         	 if(PosAni>0){//require the L always faster than R
	   		 //cout<<"check 2posani | ";//test---
		    		...
		    		tflag=1;
				//temp_writepara(ftemp,para1,ttmodel,countacc,i,-1);//---test
		    		continue;
            		}
	 	...
	 	if(Rmonoc+Lmonoc>0){
	    		...
	    		tflag=2;	
			//temp_writepara(ftemp,para1,ttmodel,countacc,i,-2);//---test
    	    		continue;}
        	}//if monoc 
		...
		*/
		...
		if(misfitcri<-1){
	  	    if(newL<oldL){
	   		...
	   		prandom=gen_random_unif01();
	    		if(prandom<prob) { 
		    		tflag=3;
				//temp_writepara(ftemp,para1,ttmodel,countacc,i,-3);//---test
		    		continue; }
	  	    }	
		...
		#pragma omp critical (updateflag_write_model)
		{
		  ...
		  para2mod_static(RApara,tmodel,RAmodel);
		  updatemodel(RAmodel,flagupdaterho);
		  RApara.parameter[6]=para1.parameter[6];//################# this is a test ###### this is added for the convience of later average process (this is the only way to seperate different phi groups)
		  write_bin(RAmodel,outbinRA,RApara,1,i,iaccp); 
		  RApara.parameter[6]=0.; //################# this is a test ######
		  }
  ...
  printf("TOTAL inv time used wtime=%.2fs cputime=%.2fs\n",omp_get_wtime()-start_walltime,get_cpu_time()-start_cputime);
