...
int main(int argc, char *argv[]){
  ...
  for(int iper=0;iper<Nper;iper++){
	per=perlst[iper];
  	printf("test-- per  = %d\n",per);
	int count=0;
  	#pragma omp parallel for default(none) shared(count,per,datadir,evnmlst,evlolst,evlalst,npts_x,npts_y,codedir,stanumcri,snrcri1,snrcri2,Nevt,lonmin,lonmax,latmin,latmax) private(str,evnm,datanm,rmstr,evlo,evla) num_threads(1)
	for(int ievt=0;ievt<Nevt;ievt++){
		...
		if((do_travel_time_to_slow_map(evnm,evlo,evla,per,0.2,lonmin,npts_x,latmin,npts_y,200.,datadir,datadir))!=1){
