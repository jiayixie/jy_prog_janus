...
// this version(v2), on Nov24, 2014, modified the compute_misfitDISP, changed the likelihood and misfit functions
	...
	{// require the L to be always faster than R in group vmono[?]
	  ...
	  for(id=vmono.begin();id<vmono.end();id++){
		...
		for(i=0;i<model.groups[j].nlay;i++){
			...
			//if(model.groups[j].vshvalue1[i]>model.groups[j].vsvvalue1[i] or (model.groups[j].vphvalue1[i]>model.groups[j].vpvvalue1[i])){// negativeAni requirement ----
