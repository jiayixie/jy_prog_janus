...
{ 
  ...
  if(argc!=4)
    {
      printf("argc=%d\n",argc);
      cout<<" usage:lfroate stalist1 stalst2 path"<<endl;
      return 0;
    }
  FILE *f1,*f2,*f3;
  int N=1000,N1,N2;
  ...
  for(i=0;i<N;i++)
    {
      if(fscanf(f1,"%s %g %g",name1[i],&dump1,&dump2)==EOF) break;
      cout<<name1[i]<<endl;
    }
  fclose(f1);
  N1=i;
  ...
  for(i=0;i<N;i++)
    {
      if(fscanf(f1,"%s %g %g",name2[i],&dump1,&dump2)==EOF) break;
      cout<<name2[i]<<endl;
    }
  fclose(f1);
  N2=i;
  ...
  for(j=0;j<N1;j++)
    {
      cout<<"working on sta "<<j<<" "<<name1[j]<<" "<<endl;
      sprintf(buff,"if [ ! -d STACK_TT/%s ]; then mkdir STACK_TT/%s; fi",name1[j],name1[j]);
      system(buff);
      sprintf(buff,"if [ ! -d STACK_RR/%s ]; then mkdir STACK_RR/%s; fi",name1[j],name1[j]);
      system(buff);
      sprintf(buff,"if [ ! -d STACK_TR/%s ]; then mkdir STACK_TR/%s; fi",name1[j],name1[j]);
      system(buff);
      sprintf(buff,"if [ ! -d STACK_RT/%s ]; then mkdir STACK_RT/%s; fi",name1[j],name1[j]);
      system(buff);
      for(k=0;k<N2;k++)
	{
	  //cout<<"working on "<<name[j]<<" "<<name[k]<<endl;
	  sprintf(name_EE,"%s/%s/COR_%s_%s.SAC_EE",arg[3],name1[j],name1[j],name2[k]);
	  sprintf(name_EN,"%s/%s/COR_%s_%s.SAC_EN",arg[3],name1[j],name1[j],name2[k]);
	  sprintf(name_NE,"%s/%s/COR_%s_%s.SAC_NE",arg[3],name1[j],name1[j],name2[k]);
	  sprintf(name_NN,"%s/%s/COR_%s_%s.SAC_NN",arg[3],name1[j],name1[j],name2[k]);
	  sprintf(name_TT,"./STACK_TT/%s/COR_%s_%s.SAC_TT",name1[j],name1[j],name2[k]);
	  sprintf(name_RR,"./STACK_RR/%s/COR_%s_%s.SAC_RR",name1[j],name1[j],name2[k]);
	  sprintf(name_TR,"./STACK_TR/%s/COR_%s_%s.SAC_TR",name1[j],name1[j],name2[k]);
	  sprintf(name_RT,"./STACK_RT/%s/COR_%s_%s.SAC_RT",name1[j],name1[j],name2[k]);
