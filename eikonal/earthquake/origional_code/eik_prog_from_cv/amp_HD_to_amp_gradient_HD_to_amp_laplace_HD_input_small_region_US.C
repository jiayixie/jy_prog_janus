...
//  y0=25;
  ...
  for(;;)
      ...
      for(;;)
	{
      ...
      fclose(fgradx);
      fclose(fgrady);
      sprintf(buff1,"~/PROGS_64/EARTHQUAKES/C_plot_travel %s_am_gradx.txt_v2 %g %g %g %g",event_name,x0,x1,y0,y1);
      system(buff1);
      sprintf(buff1,"~/PROGS_64/EARTHQUAKES/C_plot_travel %s_am_grady.txt_v2 %g %g %g %g",event_name,x0,x1,y0,y1);
      system(buff1);
