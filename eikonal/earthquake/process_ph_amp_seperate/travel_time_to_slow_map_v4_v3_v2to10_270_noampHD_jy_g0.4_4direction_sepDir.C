...
int main(int na, char *arg[])
{
  if(na!=6)
    {
      cout<<"usage:travel_time_to_slow_map event.lst period dist_cri dx dirph"<<endl;
      cout<<na<<endl; 
     return 0;
    }
  ...
  int npts_x,npts_y;
  char buff1[300],sta1[10],dirph[200];
  ...
  double period,dist,dump1,dump2,dump3,dump4;
  int nn,nsta;
  float dist_cri;
  period=atof(arg[2]);
  dist_cri = atof(arg[3]);
...
//  npts_x=63;
//  npts_y=61;
  dx=atof(arg[4]);//degree
  dy=atof(arg[4]);//degree
  sprintf(dirph,"%s",arg[5]);
 /* x0=atof(arg[6]);
  x1=atof(arg[7]);
  y0=atof(arg[8]);
  y1=atof(arg[9]);
 ...
 y1=-999;
  ...
  sprintf(buff1,"%s/%s.ph.txt_v1.HD",dirph,event_name);
...
//      if(fscanf(file1,"%s %lf %lf %lf %lf",&event_name,&dump1,&dump2,&dump3,&dump4)==EOF)
      ...
      if(access(buff1, F_OK) == 0)
	{
      ...
      sprintf(buff1,"%s/%s.ph.txt_v2.HD",dirph,event_name);
      if((fin=fopen(buff1,"r"))==NULL)
       {
	 cout<<buff1<<" not exist!!"<<endl;
	 return 1;
       }
      sprintf(buff1,"%s/%s.ph.txt_v2.HD_0.2",dirph,event_name);
      ...
      for(i=0;i<npts_x;i++)
	{
      ...
      sprintf(buff1,"%s/%s.ph.txt_v2",dirph,event_name);
      if((fin3=fopen(buff1,"r"))==NULL)
	{
      ...
      for(;;)
	{
	  if(fscanf(fin3,"%lf %lf %lf %lf",&sta1_lon[nsta],&sta1_lat[nsta],&temp2,&temp3)==EOF) 
	    break;
	  nsta++;
	  if(nsta>=1000)
	    {
      ...
      fclose(fin3);
      for(;;)  // get travel time info for each point
	{
	  if(fscanf(fin,"%lf %lf %lf",&lon,&lat,&temp)==EOF) break; 
	  ...
	  i=int((lon-x0)/dx+0.1);
	  j=int((lat-y0)/dy+0.1);
	  if(temp<temp2-1||temp>temp2+1||temp<2*period ) //PARAMETER dt num*T
// 1) if travel time from HD Tension=0 and 0.2 differs more than 1s, ingore this point; ALSO, if point is within 2 period region, set tr_t[i][j]=0
	    {
	      tr_t[i][j]=0;	     
	      continue;
	    }
	  marker_nn=4;		// 2) within some distance criteria, If there are less than 3 stations in 3 different quadrant, set tr_t[i][j]=0 for this point
	  marker_EN[0][0]=0;
	  marker_EN[0][1]=0;
	  marker_EN[1][0]=0;
          marker_EN[1][1]=0;
	  ...
	  for(ii=0;ii<=nsta;ii++)
		...
		continue;
	      ...
	      if(dist < dist_cri) // PARAMETER distance criteria
		{
		  marker_nn--;
		  if(marker_nn==0)
		    {
      ...
      for(i=4;i<npts_x-4;i++)
	{
	  for(j=4;j<npts_y-4;j++)
	      ...
	      if(temp2==0)
		{
	      ...
	      temp=sqrt(temp1*temp1+temp2*temp2);//slowness
...
//printf("0.1<%lf<0.5?\n",temp);
		...
		{ // 3) if slowness not within [0.1,0.5] OR if one of the nearby four points was ignored, slowness=0 azi=999
		  slow_all[i][j][0]=0;
		  slow_all[i][j][1]=999;
		  slow_all[i][j][2]=999;
		  slow_all[i][j][3]=999;
      ...
      for(i=4;i<npts_x-4;i++) // 4) for the nearby 9 points, at least 4 (>3) points satisfy the previous criteria (slowness !=0), output result, otherwise, slowness=0 azi = 999
	{
	  for(j=4;j<npts_y-4;j++)
	    {
	      if(slow_all[i][j][0]==0)
